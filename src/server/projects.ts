...
export const createProject = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let slug = base;
    for (let i = 2; await db.query.project.findFirst({ where: eq(project.slug, slug) }); i++) {
      slug = `${base}-${i}`;
    }
    return db.transaction(async (tx) => {
      const [created] = await tx.insert(project).values({ name: data.name, slug }).returning();
...
export const getProjectSettings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const pages = await db
      ...
      .from(page)
      .where(and(eq(page.projectId, row.id), eq(page.status, "published"), sql`${page.deletedAt} is null`))
      .orderBy(page.name);
    return {
      name: row.name,
      slug: row.slug,
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.homePageId !== undefined || data.notFoundPageId !== undefined)
      patch.settings = {
        ...
        ...(data.notFoundPageId !== undefined ? { notFoundPageId: data.notFoundPageId } : {}),
      };
    const slug = data.slug;
    const slugChanged = slug !== undefined && slug !== row.slug;
    if (slugChanged) {
      if (!isValidSlug(slug)) throw new Error("Endereço inválido: use letras minúsculas, números e hífen");
      const taken = await db.query.project.findFirst({
        where: eq(project.slug, slug),
      });
      if (taken) throw new Error("Esse endereço já está em uso por outro projeto");
      patch.slug = slug;
    }
    await db.transaction(async (tx) => {
      ...
      if (slugChanged) {
        ...
        await tx
          ...
          .where(and(eq(page.projectId, row.id), isNotNull(page.publishedHtml)));
      }
    });
    return { ok: true };
  });
