...
function leadWhere(f: LeadFilters) {
	...
	return and(
		...
		f.q
			? sql`${formSubmission.data}::text ilike ${`%${f.q.replace(/[%_]/g, "\\$&")}%`}`
			: undefined,
	);
}
...
function orderColumns(keys: Iterable<string>) {
	const all = [...new Set(keys)].filter(
		(k) => !/^(utm_|fbclid|gclid|tags$)/.test(k),
	);
	return all.sort((a, b) => {
...
export const exportLeadsCsv = createServerFn({ method: "GET" })
	...
	.handler(async ({ data, context }) => {
		...
		const columns = [
			...orderColumns(fields),
			...fields.filter((k) => /^(utm_|fbclid|gclid|tags$)/.test(k)).sort(),
		];
		const header = ["Data", "Página", "Formulário", ...columns];
		const lines = rows.map((r) =>
			[
				r.createdAt.toLocaleString("pt-BR", { timeZone: TZ }),
				r.pageName,
				r.formName,
...
export const deleteLead = createServerFn({ method: "POST" })
	...
	.handler(async ({ data, context }) => {
		...
		await db
			...
			.where(
				and(
					eq(formSubmission.id, data.id),
					eq(formSubmission.projectId, data.projectId),
				),
			);
		return { ok: true };
	});
...
function refHost(ref: unknown): string | null {
	...
	try {
		const parts = new URL(ref).hostname
			.split(".")
			.filter((p) => !HOST_NOISE.has(p));
...
export const getInsights = createServerFn({ method: "GET" })
	...
	.handler(async ({ data, context }) => {
		...
		const pagesOfProject = and(
			eq(page.projectId, data.projectId),
			data.pageId ? eq(page.id, data.pageId) : undefined,
		);
		...
		const [viewsByDay, leadsByDay, byPage, leadsByPage, metas] =
			await Promise.all([
				db
				...
				db
					...
					.leftJoin(
						pageView,
						and(eq(pageView.pageId, page.id), gte(pageView.createdAt, from)),
					)
		...
		for (const { meta } of metas) {
			...
			const source =
				(query.utm_source || refHost(meta.ref))?.toLowerCase() || "Direto";
