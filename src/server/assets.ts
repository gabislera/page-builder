...
const ALLOWED = /^(image\/(png|jpe?g|webp|gif|svg\+xml|avif|x-icon|vnd\.microsoft\.icon)|video\/(mp4|webm))$/;
...
export const registerAsset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await requireProjectAccess(context.user.id, data.projectId);
    if (!data.key.startsWith(`${data.projectId}/`)) throw new Error("Chave inválida");
    const [row] = await db
      .insert(asset)
...
export const deleteAsset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [row] = await db
      .delete(asset)
      .where(and(eq(asset.id, data.assetId), eq(asset.projectId, data.projectId)))
      .returning();
