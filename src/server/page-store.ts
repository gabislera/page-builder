...
import { isTopBar, mergePage, type SectionTree, type SitePart } from "#/builder/core/tree";
...
export async function renderTemplatePreview(projectId: string, template: PageTemplate) {
  ...
  const [header, footer] = await Promise.all([
    template.headerMode === "site" ? loadSitePart(projectId, settings.headerSectionId, "header") : null,
    template.footerMode === "site" ? loadSitePart(projectId, settings.footerSectionId, "footer") : null,
  ]);
...
export async function uniqueSlug(projectId: string, base: string) {
  ...
  for (let i = 2; ; i++) {
    const taken = await db.query.page.findFirst({
      where: and(eq(page.projectId, projectId), eq(page.slug, slug), isNull(page.deletedAt)),
    });
...
export async function upsertSections(tx: Tx, projectId: string, sections: SectionTree[]) {
...
export async function insertSections(tx: Tx, projectId: string, pageId: string, sections: SectionTree[]) {
