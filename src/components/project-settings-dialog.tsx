		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>
						O endereço do projeto é o começo do link de todas as páginas.
					</DialogDescription>
				</DialogHeader>
				{!d ? (
						...
						<div className="flex flex-col gap-1.5">
							...
							{slugChanged ? (
								<p className="text-xs leading-relaxed text-amber-500">
									Os links das páginas passam a usar o endereço novo, e os links
						...
						{d.isOwner ? (
							<div className="mt-2 flex items-center justify-between gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-3">
								<div className="flex flex-col gap-0.5">
									...
									<span className="text-xs text-muted-foreground">
										Apaga páginas, leads, visitas e arquivos. Não dá para
										desfazer.
									</span>
								</div>
								<Button
									...
									onClick={async () => {
										const ok = await confirm({
											...
											description: (
												<>
													<strong>{d.name}</strong> e tudo o que há nele
													(páginas, leads, visitas e arquivos) serão apagados
													para sempre.
												</>
											),
				...
				<DialogFooter>
					...
					<Button
						...
						disabled={
							!d || save.isPending || name.trim().length < 2 || !slugify(slug)
						}
					>
						{save.isPending ? (
