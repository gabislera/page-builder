		...
		<Dialog open={Boolean(page)} onOpenChange={(v) => !v && onClose()}>
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>
						O nome é só para você se organizar. O endereço é o que aparece no
						link da página.
					</DialogDescription>
				</DialogHeader>
				<form
					...
					<div className="flex flex-col gap-1.5">
						...
						{changedSlug && page?.status === "published" ? (
							<p className="text-xs leading-relaxed text-amber-500">
								A página publicada passa a responder no endereço novo, e o
								endereço antigo deixa de funcionar.
							</p>
						) : null}
					</div>
				</form>
				<DialogFooter>
					...
					<Button
						type="submit"
						form="page-address"
						disabled={save.isPending || !name.trim() || !slugify(slug)}
					>
						{save.isPending ? (
