...
import { Download, ExternalLink, Inbox, Loader2, MessageCircle, Search, Trash2 } from "lucide-react";
...
function LeadsPage() {
  ...
  const leads = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => listLeads({ data: { ...filters, offset: pageParam } }),
    initialPageParam: 0,
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button variant="outline" disabled={!first?.total || exportCsv.isPending} onClick={() => exportCsv.mutate()}>
          {exportCsv.isPending ? <Loader2 className="size-4 animate-spin" /> : <Download className="size-4" />}
          Exportar CSV
        </Button>
      </div>

      <p className="text-sm text-muted-foreground">
        {first ? `${first.total.toLocaleString("pt-BR")} ${first.total === 1 ? "lead" : "leads"}` : "Carregando..."}
      </p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((lead) => (
                ...
                >
                  <td className="px-4 py-3 whitespace-nowrap text-muted-foreground">{dateTime(lead.createdAt)}</td>
                  {columns.map((c) => (
                    <td key={c} className="max-w-56 truncate px-4 py-3">
                      {display(c, lead.data[c]) || <span className="text-muted-foreground">—</span>}
                    </td>
                  ))}
                  <td className="max-w-44 truncate px-4 py-3 text-muted-foreground">{lead.pageName}</td>
      ...
      {leads.hasNextPage ? (
        ...
        >
          {leads.isFetchingNextPage ? <Loader2 className="size-4 animate-spin" /> : null}
          Carregar mais
        </Button>
      ) : null}
      ...
      >
        ...
        {tracking.length || lead.url ? (
          <div className="flex flex-col gap-2 rounded-lg border border-border bg-card/50 p-3">
            <span className="text-xs font-medium text-muted-foreground">Rastreamento</span>
