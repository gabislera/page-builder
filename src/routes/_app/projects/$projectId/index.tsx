...
import { CloudOff, Copy, ExternalLink, Link2, Loader2, MoreHorizontal, Pencil, Trash2 } from "lucide-react";
...
import { deletePage, duplicatePage, listPages, unpublishPage } from "#/server/pages";
...
function ProjectPages() {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ["pages", projectId] });
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-4">
        <p className="text-sm text-muted-foreground">
          {pages.data ? `${pages.data.length} ${pages.data.length === 1 ? "página" : "páginas"}` : ""}
        </p>
        <NewPageDialog projectId={projectId} projectSlug={project?.slug ?? ""} />
      </div>
      {pages.isLoading ? <Loader2 className="size-5 animate-spin text-muted-foreground" /> : null}
      {pages.data?.length === 0 ? (
      ...
      <div className="overflow-hidden rounded-xl border border-border">
        {pages.data?.map((p) => (
          <div key={p.id} className="flex items-center gap-4 border-b border-border bg-card px-5 py-3 last:border-0">
            <div className="flex min-w-0 flex-1 flex-col">
              <Link to="/editor/$pageId" params={{ pageId: p.id }} className="truncate font-medium hover:text-primary">
                {p.name}
              </Link>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {p.status === "published" && project ? (
                  <DropdownMenuItem asChild>
                    <a href={`/p/${project.slug}/${p.slug}`} target="_blank" rel="noreferrer">
                  ...
                  <DropdownMenuItem
                    onClick={async () => {
                      const ok = await confirm({
                        ...
                        description: (
                          <>
                            <strong>{p.name}</strong> sai do ar: o endereço deixa de abrir e os formulários param de
                            receber leads. O conteúdo continua salvo.
                          </>
                        ),
                    ...
                    const ok = await confirm({
                      ...
                      description: (
                        <>
                          ...
                          {p.status === "published" ? " e sairá do ar" : ""}. Essa ação não pode ser desfeita.
                        </>
                      ),
