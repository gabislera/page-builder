  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {insights.isFetching ? <Loader2 className="size-4 animate-spin text-muted-foreground" /> : null}
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <Kpi label="Conversão" value={d ? pct(d.totals.conversion) : "—"} hint="Leads ÷ visitas no período" />
      </div>

      <Panel title="Visitas e leads por dia">{d ? <DailyChart daily={d.daily} /> : <div className="h-56" />}</Panel>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                ...
                {d.pages.map((p) => (
                  <tr key={p.id} className="border-t border-border">
                    <td className="max-w-64 truncate py-2.5">
                      <Link to="/editor/$pageId" params={{ pageId: p.id }} className="hover:text-primary">
                        {p.name}
                      </Link>
                      {p.status !== "published" ? (
                    ...
                    <td className="py-2.5 text-right tabular-nums">{n(p.views)}</td>
                    <td className="py-2.5 text-right tabular-nums">{n(p.leads)}</td>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Panel title="Dispositivos">
            {d?.totals.views ? <Devices mobile={d.devices.mobile} desktop={d.devices.desktop} /> : <Empty />}
          </Panel>
        </div>
      </div>
    </div>
...
function Kpi({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <div className="flex flex-col gap-1 rounded-xl border border-border bg-card p-4" title={hint}>
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-4 text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5">
          <span className="size-2.5 rounded-sm bg-muted-foreground/40" /> Visitas
        </span>
        <span className="flex items-center gap-1.5">
          ...
          >
            ...
            >
              <div
                ...
                style={{
                  height: x.views ? `${Math.min(100, (x.leads / x.views) * 100)}%` : 0,
      ...
      {items.map((i) => (
        <li key={i.label} className="flex flex-col gap-1 text-sm">
          <div className="flex justify-between gap-2">
            ...
            <span className="text-muted-foreground tabular-nums">{n(i.value)}</span>
          </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex h-2 overflow-hidden rounded-full bg-muted">
        <div className="bg-primary" style={{ width: `${(mobile / total) * 100}%` }} />
      </div>
