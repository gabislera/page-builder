...
function ProgressBarView({
	...
	rootRef,
	onPropChange,
}: NodeViewProps<ProgressBarProps>) {
		...
		>
			{head ? (
				<div className="pb-progress-head">
					...
					{p.labelPosition === "outside" ? (
						<span className="pb-progress-pct">{pct}</span>
					) : null}
				</div>
			) : null}
			<div
...
function ProgressBarSettings() {
	...
	const label =
		useField<ProgressBarProps["labelPosition"]>("labelPosition").value;
			...
			content={
				<>
					<Group title="Barra">
						...
						{showTitle ? (
							<TextField
								path="title"
								label="Título"
								hint="Dica: dê dois cliques no título para editar no canvas."
							/>
						) : null}
					...
					<Group title="Animação">
						...
						{striped ? (
							<SwitchField
								path="stripesAnimated"
								label="Listras em movimento"
							/>
						) : null}
					</Group>
				</>
				...
				<>
					...
					{label !== "none" ? (
						<Group title="Porcentagem" defaultOpen={false}>
							<TypographyFields
								base="labelTypography"
								withAlign={false}
								withColor={label === "outside"}
							/>
							{label === "inside" ? (
								<ColorField path="insideLabelColor" label="Cor" />
							) : null}
						</Group>
					) : null}
				</>
...
export const ProgressBar: ComponentDefinition<ProgressBarProps> = {
	...
	css: (id, p) => {
		...
		sheet
			.rule(" .pb-progress-head .pb-progress-pct")
		...
		const layers = [
			p.striped ? STRIPES : null,
			p.fill === "gradient" ? gradientToCss(p.barGradient) : null,
		].filter(Boolean);
