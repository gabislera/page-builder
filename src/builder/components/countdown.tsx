...
import { DateTimeField, TIMEZONE_OPTIONS, TimeField } from "../controls/datetime.tsx";
import { Group } from "../controls/field.tsx";
import { BorderFields, BoxFields, NumberField, SidesField, TypographyFields } from "../controls/groups.tsx";
import {
  NumberUnitField,
  SegmentedField,
  SelectField,
...
import { corners, defaultBorder, defaultBox, defaultTypography, sides } from "../core/defaults.ts";
...
import { applyBorder, applyBox, applyTypography, createSheet, sidesToCss } from "../core/style-engine.ts";
import type { Border, Box, Length, Sides, Typography } from "../core/style-types.ts";
...
function CountdownView({ id, props: p, rootRef }: NodeViewProps<CountdownProps>) {
  ...
  if (p.mode === "date") {
    ...
    data["data-pb-duration"] = String(Math.max(0, Math.round(p.durationMinutes * 60)));
  } else {
    data["data-pb-daily"] = p.dailyTime;
    data["data-pb-tz"] = p.timezone;
  }
  if (p.expireAction === "redirect" && p.expireUrl) data["data-pb-redirect"] = p.expireUrl;
        ...
        {visible.map((u, i) => (
          <span key={u} className="pb-cd-group">
            ...
            <span className="pb-cd-unit">
              ...
              {p.showLabels ? <span className="pb-cd-label">{p.labels[u]}</span> : null}
            </span>
          </span>
        ))}
      </div>
...
function UnitsFields() {
  ...
  return (
    <>
      ...
      {showLabels
        ? UNITS.map((u) => <TextField key={u} path={`labels.${u}`} label={`Rótulo: ${UNIT_LABEL[u].toLowerCase()}`} />)
        : null}
    </>
  );
}
      ...
      content={
        <>
          <Group title="Contagem">
            ...
            />
            {mode === "date" ? <DateTimeField path="endDate" label="Termina em" /> : null}
            {mode === "evergreen" ? (
              <NumberField path="durationMinutes" label="Duração (minutos)" min={1} max={43200} />
            ) : null}
            {mode === "daily" ? <TimeField path="dailyTime" label="Zera às" /> : null}
            {mode !== "evergreen" ? (
              <SelectField path="timezone" label="Fuso horário" options={TIMEZONE_OPTIONS} />
            ) : (
              <p className="text-[11px] text-muted-foreground">
                Cada visitante tem sua própria contagem, que continua de onde parou ao recarregar a página.
              </p>
            )}
          </Group>
          ...
          {mode !== "daily" ? (
            <Group title="Ao terminar">
              ...
              />
              {expire === "show-message" ? <TextAreaField path="expireMessage" label="Mensagem" /> : null}
              {expire === "redirect" ? <TextField path="expireUrl" label="Link" placeholder="https://..." /> : null}
            </Group>
          ) : null}
        </>
      }
      style={
        <>
          <Group title="Layout">
            ...
            />
            <NumberUnitField path="gap" label="Espaço entre unidades" units={["px", "rem"]} max={80} />
          </Group>
          <Group title="Caixa de cada unidade">
            <ColorField path="unitBackground" label="Fundo" allowEmpty />
            <SidesField path="unitPadding" label="Espaço interno" units={["px", "rem"]} />
            <NumberUnitField path="unitMinWidth" label="Largura mínima" units={["px", "rem"]} max={300} />
...
export const Countdown: ComponentDefinition<CountdownProps> = {
  ...
  defaults: {
    ...
    unitPadding: responsive(sides("14px", "12px"), undefined, sides("10px", "8px")),
  ...
  css: (id, p) => {
    ...
    sheet
      ...
      .set("gap", p.gap);
    sheet.rule(" .pb-cd-group").set("display", "flex").set("align-items", "flex-start").set("gap", p.gap);
  ...
  runtime: ["countdown"],
  fonts: (p) => [p.numberTypography.fontFamily, p.labelTypography.fontFamily, p.messageTypography.fontFamily],
};
