...
import { BorderFields, BoxFields, MediaPathField, NumberField, ShadowFields } from "../controls/groups.tsx";
import { NumberUnitField, SelectField, SwitchField, TextAreaField, TextField } from "../controls/inputs.tsx";
...
import { applyBorder, applyBox, createSheet, shadowToCss } from "../core/style-engine.ts";
...
export function vimeoId(url: string): { id: string; hash?: string } | null {
  ...
  if (!m) return null;
  const hash = m[2] ?? new URLSearchParams(u.split("?")[1] ?? "").get("h") ?? undefined;
  return { id: m[1], hash };
}
...
function aspectCss(p: VideoProps): string | undefined {
  ...
  const m = /^\s*(\d+(?:\.\d+)?)\s*[/:x]\s*(\d+(?:\.\d+)?)\s*$/.exec(p.customAspect);
...
function VideoView({ id, props: p, rootRef }: NodeViewProps<VideoProps>) {
  ...
  const thumb = p.thumbnail || (ytId ? `https://i.ytimg.com/vi/${ytId}/hqdefault.jpg` : "");
  ...
  if (p.source === "youtube" || p.source === "vimeo") {
    ...
    if (!embedSrc) {
      ...
      content = (
        ...
        >
          <img src={thumb} alt="" loading={isEditor ? undefined : "lazy"} decoding="async" />
          <PlayIcon />
        </button>
      );
    } else {
  ...
  if (empty) {
    ...
    return (
      <div ref={rootRef as React.Ref<HTMLDivElement>} className={className} data-pb-node={id}>
        <div className="pb-placeholder">Cole o link do vídeo no painel ao lado</div>
      </div>
    );
  }
          ...
          <Group title="Vídeo">
            ...
            {isPlayer ? (
              <TextField
                path="url"
                label="Link do vídeo"
                placeholder={source === "youtube" ? "https://www.youtube.com/watch?v=..." : "https://vimeo.com/..."}
                hint="Cole o link do navegador ou de compartilhamento."
              />
            ) : null}
            {source === "upload" ? <MediaPathField path="file" label="Arquivo" accept="video" /> : null}
            {source === "embed" ? (
              <TextAreaField
                path="embedCode"
                label="Código"
          ...
          {source !== "embed" ? (
            <Group title="Reprodução">
              <SwitchField
                ...
                hint={autoplay ? "Com reprodução automática o vídeo começa sem som." : undefined}
              />
              <SwitchField path="muted" label="Sem som" />
              <SwitchField path="loop" label="Repetir" />
              <SwitchField path="controls" label="Mostrar controles" />
              <SwitchField path="playsinline" label="Tocar na página (iPhone)" />
              {isPlayer ? <NumberField path="start" label="Começar em (segundos)" max={36000} /> : null}
            </Group>
          ) : null}
          {source !== "embed" ? (
      ...
      style={
        <>
          <Group title="Dimensões">
            ...
            />
            {aspect === "custom" ? (
              <TextField path="customAspect" label="Proporção personalizada" placeholder="21/9" />
            ) : null}
            <NumberUnitField
              path="box.maxWidth"
          ...
          {isPlayer ? (
            <Group title="Botão de play" defaultOpen={false}>
              <NumberUnitField path="playIconSize" label="Tamanho" units={["px"]} max={200} />
...
export const Video: ComponentDefinition<VideoProps> = {
  ...
  css: (id, p) => {
    ...
    for (const tag of ["iframe", "video", "embed"]) {
      sheet.rule(` .pb-video-embed ${tag}`).set("width", "100%").set("height", "100%").set("border", "0");
    }
    sheet
    ...
    sheet.rule(" .pb-video-facade:hover .pb-video-play").set("transform", "translate(-50%, -50%) scale(1.08)");
    sheet.rule(" .pb-video-shield").set("position", "absolute").set("inset", "0").set("z-index", "2");
