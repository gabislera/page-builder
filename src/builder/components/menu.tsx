...
import { BoxFields, ShadowFields, SidesField, TypographyFields } from "../controls/groups.tsx";
...
import { defaultBox, defaultShadow, defaultTypography, sides } from "../core/defaults.ts";
...
import type { Action, Box, Length, Shadow, Sides, Typography } from "../core/style-types.ts";
...
function MenuView({ id, props, rootRef, onPropChange }: NodeViewProps<MenuProps>) {
  ...
  const toggleSub = (itemId: string) =>
    setOpenSubs((list) => (list.includes(itemId) ? list.filter((x) => x !== itemId) : [...list, itemId]));
    ...
    >
      <ul className="pb-menu-list">
        {props.items.map((item, i) => {
          ...
          return (
            <li key={item.id} className={hasSub ? "pb-menu-item pb-has-sub" : "pb-menu-item"}>
              <a className="pb-menu-link" {...linkOf(item.action)}>
                <InlineText value={item.text} onCommit={commit(`items.${i}.text`)} />
                {hasSub && !vertical ? <IconView name="chevron-down" className="pb-menu-caret" /> : null}
              </a>
              {hasSub ? (
                <ul className="pb-submenu">
                  {item.children.map((sub, j) => (
                    <li key={sub.id}>
                      <a className="pb-menu-link pb-submenu-link" {...linkOf(sub.action)}>
                        <InlineText value={sub.text} onCommit={commit(`items.${i}.children.${j}.text`)} />
                      </a>
                    </li>
                  ))}
                </ul>
        ...
        <>
          ...
          <div className="pb-menu-panel">
            <div
              ...
              aria-hidden="true"
            />
            <div className="pb-menu-drawer" role="dialog" aria-modal="true" aria-label="Menu">
              <button
                type="button"
              ...
              <ul className="pb-menu-mlist">
                {props.items.map((item) => {
                  ...
                  return (
                    <li key={item.id} className={subOpen ? "pb-sub-open" : undefined}>
                      <div className="pb-menu-mrow">
                        ...
                        {hasSub ? (
                          <button
                            ...
                            onClick={isEditor ? () => toggleSub(item.id) : undefined}
                          >
                      ...
                      {hasSub ? (
                        <ul className="pb-menu-msub">
                          {item.children.map((sub) => (
                            <li key={sub.id}>
                              <a className="pb-menu-mlink" {...linkOf(sub.action)}>
                                {sub.text}
                              </a>
                            </li>
                          ))}
      ...
      style={
        <>
          <Group title="Links">
            ...
            <ColorField path="activeColor" label="Cor da página atual" allowEmpty />
            <NumberUnitField path="gap" label="Espaço entre links" units={["px", "rem"]} max={80} />
            <SidesField path="itemPadding" label="Espaço interno dos links" units={["px", "em"]} />
          </Group>
          <Group title="Indicador" defaultOpen={false}>
            ...
            />
            {indicator.value !== "none" ? <ColorField path="indicatorColor" label="Cor do indicador" /> : null}
            {indicator.value === "background" ? (
              <NumberUnitField path="itemRadius" label="Arredondamento" units={["px"]} max={40} />
            ) : null}
          </Group>
          {horizontal ? (
            <Group title="Submenu" defaultOpen={false}>
              ...
              <ColorField path="dropdownHoverColor" label="Texto no hover" allowEmpty />
              <ColorField path="dropdownHoverBackground" label="Fundo no hover" allowEmpty />
              <NumberUnitField path="dropdownWidth" label="Largura mínima" units={["px"]} max={400} />
              <NumberUnitField path="dropdownRadius" label="Arredondamento" units={["px"]} max={32} />
          ...
          {collapsible ? (
            <Group title="Menu no celular" defaultOpen={false}>
              ...
              <NumberUnitField path="toggleSize" label="Tamanho do ícone" units={["px"]} max={64} />
              <ColorField path="panelBackground" label="Fundo do painel" />
              <ColorField path="panelTextColor" label="Cor dos links" />
              <NumberUnitField path="panelFontSize" label="Tamanho dos links" units={["px", "rem"]} max={40} />
              <NumberUnitField path="panelWidth" label="Largura da gaveta" units={["px", "vw"]} max={600} />
            </Group>
          ) : null}
        </>
      }
...
function menuCss(id: string, p: MenuProps): string {
  ...
  list
    ...
    .set("padding", "0");
  if (vertical) {
    list.set("align-items", p.align, (v) => (v === "space-between" ? "stretch" : v));
  } else {
  ...
  link
    ...
    .set("cursor", "pointer")
    .set("transition", `color ${TRANSITION},background-color ${TRANSITION},opacity ${TRANSITION}`);
  applyTypography(link, { ...p.typography, textAlign: undefined });
  sheet.rule(" .pb-menu-link:hover").set("color", p.hoverColor || undefined);
  sheet.rule(" .pb-menu-item:focus-within > .pb-menu-link").set("color", p.hoverColor || undefined);
  sheet.rule(" .pb-menu-link.pb-active").set("color", p.activeColor || undefined);
  ...
  sheet
    ...
    .set("transition", `opacity .2s ease,transform .2s ease,visibility 0s linear .2s`);
  ...
  drawer
    ...
    .set("font-family", p.typography.fontFamily, (v) => (v ? fontStack(v) : undefined))
  ...
  sheet
    ...
    .set("color", "inherit");
  sheet.rule(" .pb-menu-mlink.pb-active").set("color", p.activeColor || undefined);
  sheet
  ...
  sheet
    ...
    .set("height", "18px")
    .set("transition", `transform ${TRANSITION}`);
  sheet.rule(" .pb-sub-open .pb-menu-subtoggle svg").set("transform", "rotate(180deg)");
  sheet.rule(" .pb-menu-msub").set("display", "none").set("padding-left", "16px");
