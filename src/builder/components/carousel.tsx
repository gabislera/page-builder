...
function slideElements(track: HTMLElement): HTMLElement[] {
	return Array.from(track.children).filter(
		(el): el is HTMLElement =>
			el.classList.contains("pb-car-slide") && el.getClientRects().length > 0,
	);
}
...
function CarouselView({
	...
	const go = useCallback((index: number) => {
		...
		if (slide)
			track.scrollTo({ left: slideOffset(track, slide), behavior: "smooth" });
	}, []);
		...
		>
			<div className="pb-car-viewport">
				<div className="pb-car-track" ref={trackRef}>
					{children}
					{isEditor && count === 0 ? (
						<div className="pb-placeholder">Adicione slides no painel</div>
					) : null}
				</div>
				<button
					type="button"
				...
				<>
					...
					<Group title="Reprodução">
						...
						{autoplay > 0 ? (
							<SwitchField
								path="pauseOnHover"
								label="Pausar com o mouse em cima"
							/>
						) : null}
						<SwitchField
							path="loop"
...
export const Carousel: ComponentDefinition<CarouselProps> = {
	...
	rules: {
		canMoveIn: (incoming) =>
			incoming.every((n) => n.data.name === "CarouselSlide"),
	},
	View: CarouselView,
	css: (id, p) => {
		...
		sheet
			...
			.set("position", "relative")
			.set("padding", p.showArrows, (on) =>
				on && outside ? `0 calc(${p.arrowSize} + 12px)` : "0",
			);
		...
		sheet
			...
			.set(
				"flex",
				"0 0 calc((100% - (var(--pb-car-n) - 1) * var(--pb-car-gap)) / var(--pb-car-n))",
			)
			.set("min-width", "0")
...
function CarouselSlideView({
	...
	children,
	rootRef,
}: NodeViewProps<CarouselSlideProps>) {
	const isEditor = useIsEditor();
	...
	return (
		...
		>
			{children}
			{isEditor && empty ? (
				<div className="pb-placeholder">Arraste elementos para este slide</div>
			) : null}
			{isEditor ? (
				<RevealOnSelect
					id={id}
...
export const CarouselSlide: ComponentDefinition<CarouselSlideProps> = {
	...
	rules: {
		...
		canMoveIn: (incoming) =>
			incoming.every(
				(n) =>
					!TOP_LEVEL_TYPES.has(n.data.name) && !SLIDE_BLOCKED.has(n.data.name),
			),
	},
...
export const carouselSpec = (): NodeSpec =>
	h("Carousel", {}, [0, 1, 2].map(carouselSlideSpec));
