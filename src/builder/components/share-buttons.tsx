...
export const SHARE_NETWORKS: Record<
	ShareNetwork,
...
function ShareButtonsView({
	...
	props,
	rootRef,
}: NodeViewProps<ShareButtonsProps>) {
	...
	return (
		<div
			...
			data-pb-share=""
			data-pb-share-url={
				props.urlMode === "custom" && props.url.trim()
					? props.url.trim()
					: undefined
			}
		...
		>
			{props.label ? (
				<span className="pb-share-label">{props.label}</span>
			) : null}
				...
				{props.items.map((item) => {
					...
					const content = (
						<>
							...
							{withText ? (
								<span className="pb-share-text">{net.label}</span>
							) : null}
						</>
					);
					const common = {
					...
					return item.network === "copy" || item.network === "native" ? (
						<button
							...
							data-pb-copied={
								item.network === "copy" ? props.copiedText : undefined
							}
				...
				<>
					<Group title="Botões">
						<ListField<ShareItem>
							...
							itemLabel={(item) =>
								SHARE_NETWORKS[item.network]?.label ?? item.network
							}
							renderItem={(itemPath) => (
								<SelectField
									path={`${itemPath}.network`}
									label="Rede"
									options={(Object.keys(SHARE_NETWORKS) as ShareNetwork[]).map(
										(n) => ({ value: n, label: SHARE_NETWORKS[n].label }),
									)}
								/>
							)}
						/>
						<TextField
							path="label"
							label="Texto antes dos botões"
							placeholder="ex.: Compartilhe:"
						/>
					</Group>
					<Group title="O que compartilhar">
						...
						/>
						{urlMode.value === "custom" ? (
							<TextField path="url" label="Endereço" placeholder="https://" />
						) : null}
						<TextField
							path="text"
							label="Mensagem"
...
export const ShareButtons: ComponentDefinition<ShareButtonsProps> = {
	...
	css: (id, p) => {
		...
		sheet
			.rule(" .pb-share-btn:hover")
			.set("transform", "translateY(-2px)")
			.set("opacity", ".92");
		sheet
			.rule(" .pb-share-btn svg")
		...
		if (p.colorMode === "custom") {
			btn
				...
				.set("background-color", p.background || "transparent");
		} else {
