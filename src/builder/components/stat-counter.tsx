...
function StatCounterView({
	id,
	props,
	rootRef,
	onPropChange,
}: NodeViewProps<StatCounterProps>) {
	...
	return (
		...
		>
			<div className="pb-stat-number">
				{props.prefix ? (
					<span className="pb-stat-affix">{props.prefix}</span>
				) : null}
				<span
				...
				{props.suffix ? (
					<span className="pb-stat-affix">{props.suffix}</span>
				) : null}
			</div>
			{props.label || label.editing ? (
				<div
					className="pb-stat-label"
					ref={label.ref as React.Ref<HTMLDivElement>}
					{...label.attrs}
				>
			...
			content={
				<>
					<Group title="Número">
						...
						<TextField
							path="prefix"
							label="Antes do número"
							placeholder="ex.: +, R$"
						/>
						<TextField
							path="suffix"
							label="Depois do número"
							placeholder="ex.: %, mil, k"
						/>
						<NumberField
							path="decimals"
...
export const StatCounter: ComponentDefinition<StatCounterProps> = {
	...
	css: (id, p) => {
		...
		sheet
			...
			.set("text-align", p.align, (a) =>
				a === "center" ? "center" : a === "flex-end" ? "right" : "left",
			)
			.set("gap", p.gap);
