...
function ButtonView({ id, props, rootRef, onPropChange }: NodeViewProps<ButtonProps>) {
  const ctx = useRender();
  const edit = useInlineEdit(props.text, onPropChange && ((v) => onPropChange("text", v)));
  const link = actionLink(props.action, ctx);
  const icon = props.icon ? <IconView name={props.icon} className="pb-btn-icon" /> : null;
  const label = (
  ...
  return (
    <button ref={rootRef as React.Ref<HTMLButtonElement>} type="button" className={className} data-pb-node={id}>
      {content}
    </button>
  );
}
      ...
      content={
        <>
          <Group title="Botão">
            ...
            />
            <NumberUnitField path="iconSize" label="Tamanho do ícone" units={["px", "em"]} max={64} />
          </Group>
...
export const Button: ComponentDefinition<ButtonProps> = {
  ...
  css: (id, p) => {
    ...
    applyBox(sheet, { ...p.box, padding: undefined, width: undefined }, "inline-flex");
    sheet.rule(" .pb-btn-icon").set("width", p.iconSize).set("height", p.iconSize);
