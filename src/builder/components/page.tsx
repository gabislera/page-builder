...
import { applyBackground, createSheet, fontStack, nodeClass } from "../core/style-engine.ts";
...
function PageView({ id, children, rootRef }: Parameters<ComponentDefinition<PageProps>["View"]>[0]) {
  const isEditor = useIsEditor();
  const empty = !children || (Array.isArray(children) && children.length === 0);
  return (
    <div ref={rootRef as React.Ref<HTMLDivElement>} className={`pb-page ${nodeClass(id)}`} data-pb-node={id}>
      {isEditor ? (
        <EditorPageBody
          placeholder={
            empty ? (
              <div className="pb-placeholder" style={{ margin: 24, width: "auto", minHeight: 200 }}>
  ...
  return (
    <>
      ...
      <Group title="Tipografia">
        ...
        />
        <SelectField path="headingFontFamily" label="Fonte dos títulos" options={fontChoices()} />
        <NumberUnitField path="baseFontSize" label="Tamanho base" units={["px"]} min={12} max={24} />
...
export const Page: ComponentDefinition<PageProps> = {
  ...
  rules: {
    ...
    canMoveIn: (incoming, current, helpers) => {
      ...
      for (const kind of ["Header", "Footer", TOP_BAR_TYPE]) {
        ...
        const existing = children.filter((c) => c.data.name === kind && !adding.some((a) => a.id === c.id));
  ...
  css: (id, p) => {
    ...
    root
      ...
      .set("--pb-heading-font", fontStack(p.headingFontFamily === "inherit" ? p.fontFamily : p.headingFontFamily))
