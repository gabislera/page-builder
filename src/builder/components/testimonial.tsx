...
function TestimonialView({
	...
	rootRef,
	onPropChange,
}: NodeViewProps<TestimonialProps>) {
	const ctx = useRender();
		...
		rating > 0 ? (
			...
			>
				{[1, 2, 3, 4, 5].map((n) => (
					<span
						key={n}
						className={n <= rating ? "pb-testi-star is-on" : "pb-testi-star"}
					>
						<IconView name="star" />
					</span>
				))}
	...
	const quoteEl = (
		<blockquote
			ref={quote.ref as React.Ref<HTMLQuoteElement>}
			className="pb-testi-quote"
			{...quote.attrs}
		>
	...
	const meta = (
		<span className="pb-testi-meta">
			{props.name || name.editing ? (
				<span
					ref={name.ref as React.Ref<HTMLSpanElement>}
					className="pb-testi-name"
					{...name.attrs}
				>
			...
			{props.role || role.editing ? (
				<span
					ref={role.ref as React.Ref<HTMLSpanElement>}
					className="pb-testi-role"
					{...role.attrs}
				>
	...
	return (
		<figure
			...
			className={nodeClassName(
				id,
				`pb-testi pb-testi-${props.layout}`,
				props.box,
			)}
			data-pb-node={id}
		>
		...
		<SettingsTabs
			content={
				<>
					...
					<Group title="Layout">
						...
						{showQuoteIcon.value ? (
							<IconField path="quoteIcon" label="Ícone" />
						) : null}
					</Group>
				</>
			}
			style={
				<>
					...
					<Group
						title={layout.value === "bubble" ? "Balão" : "Caixa"}
						defaultOpen={false}
					>
...
export const Testimonial: ComponentDefinition<TestimonialProps> = {
	...
	css: (id, p) => {
		...
		if (bubble && p.background.type !== "none") {
			...
			const tipColor =
				p.background.type === "color" ? p.background.color : C.surface;
			const tipSide =
				align === "center"
					? "calc(50% - 10px)"
					: align === "right"
			...
			sheet
				...
				.set(
					"border-right",
					p.border.style !== "none"
						? `${p.border.width.desktop.right} ${p.border.style} ${p.border.color}`
						: undefined,
				)
				.set(
					"border-bottom",
			...
			sheet
				...
				.set("padding", align === "center" ? "0" : "0 16px");
		}
		if (bubble) {
			sheet
		...
		sheet
			...
			.set("line-height", "0")
			.set("color", p.quoteIconColor);
		sheet
			.rule(" .pb-testi-qicon svg")
			.set("width", p.quoteIconSize)
			.set("height", p.quoteIconSize);
		...
		sheet
			.rule(" .pb-testi-star")
			.set("display", "inline-flex")
			.set("color", p.starEmptyColor);
		sheet.rule(" .pb-testi-star.is-on").set("color", p.starColor);
		sheet
			.rule(" .pb-testi-star svg")
			.set("width", p.starSize)
			.set("height", p.starSize)
	...
	fonts: (p) => [
		p.quoteTypography.fontFamily,
		p.nameTypography.fontFamily,
		p.roleTypography.fontFamily,
	],
};
