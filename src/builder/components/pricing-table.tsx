...
function PricingTableView({
	...
	rootRef,
	onPropChange,
}: NodeViewProps<PricingTableProps>) {
	const ctx = useRender();
	...
	const installments = useInlineEdit(
		props.installments,
		commit("installments"),
	);
	...
	return (
		<div
			...
			className={nodeClassName(
				id,
				props.featured ? "pb-price is-featured" : "pb-price",
				props.box,
			)}
			data-pb-node={id}
		>
			{props.featured && (props.badgeText || badge.editing) ? (
				<span
					ref={badge.ref as React.Ref<HTMLSpanElement>}
					className="pb-price-badge"
					{...badge.attrs}
				>
			...
			<div className="pb-price-head">
				<NameTag
					ref={name.ref as React.Ref<HTMLHeadingElement>}
					className="pb-price-name"
					{...name.attrs}
				>
				...
				{props.description || desc.editing ? (
					<p
						ref={desc.ref as React.Ref<HTMLParagraphElement>}
						className="pb-price-desc"
						{...desc.attrs}
					>
			...
			<div className="pb-price-value">
				{props.oldPrice ? (
					<del className="pb-price-old">{props.oldPrice}</del>
				) : null}
				<div className="pb-price-row">
					{props.currency ? (
						<span className="pb-price-cur">{props.currency}</span>
					) : null}
					<span
						ref={amount.ref as React.Ref<HTMLSpanElement>}
						className="pb-price-amount"
						{...amount.attrs}
					>
						{amount.editing ? null : props.amount}
					</span>
					{props.cents ? (
						<span className="pb-price-cents">{props.cents}</span>
					) : null}
					{props.period ? (
						<span className="pb-price-period">{props.period}</span>
					) : null}
				</div>
			...
			{props.features.length ? (
				<ul className="pb-price-features">
					{props.features.map((f, i) => (
						<li
							key={f.id}
							className={f.included ? "pb-price-feat" : "pb-price-feat is-off"}
						>
							<span className="pb-price-feat-icon">
								<IconView
									name={f.included ? props.includedIcon : props.excludedIcon}
									strokeWidth={2.5}
								/>
							</span>
							{f.included ? null : <span className="pb-sr">Não inclui: </span>}
							<InlineText
								className="pb-price-feat-text"
								value={f.text}
								onCommit={commit(`features.${i}.text`)}
							/>
						</li>
					))}
				</ul>
				...
				<>
					...
					<Group title="Preço">
						...
						<TextField
							path="installments"
							label="Parcelamento"
							placeholder="ex.: ou 12x de R$ 9,70"
						/>
					</Group>
					...
					<Group title="Destaque">
						<SwitchField
							...
							hint="Selo, borda colorida e leve ampliação."
						/>
						{featured.value ? (
							<TextField
								path="badgeText"
			...
			style={
				<>
					...
					<Group title="Preço" defaultOpen={false}>
						...
						<ColorField
							path="priceMutedColor"
							label="Cor do período e parcelas"
						/>
					</Group>
	...
	css: (id, p) => {
		...
		if (p.featured) {
			...
			if (p.featuredScale !== 1) {
				root
					.setOn("desktop", "scale", String(p.featuredScale))
					.setOn("tablet", "scale", "1");
			}
			sheet
				.rule(" .pb-price-badge")
		...
		sheet
			.rule(" .pb-price-feat-icon svg")
			.set("width", "1.05em")
			.set("height", "1.05em");
		sheet
			.rule(" .pb-price-feat.is-off")
			.set("color", p.excludedColor)
			.set("opacity", ".75");
		sheet
			.rule(" .pb-price-feat.is-off .pb-price-feat-icon")
			.set("color", p.excludedColor);
		sheet
			.rule(" .pb-sr")
			.set("position", "absolute")
