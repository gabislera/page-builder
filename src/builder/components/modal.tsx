		...
		return (
			...
			>
				<span style={EDITOR_LABEL}>
					...
					{auto
						? ` · abre sozinho${props.autoOpen === "delay" ? ` após ${props.delay}s` : " na saída"}`
						: ""}
				</span>
				{trigger}
				<div
					...
					<div className="pb-modal-content">
						...
						{empty ? (
							<div className="pb-placeholder">
								Arraste elementos para dentro do modal
							</div>
						) : null}
					</div>
				</div>
	...
	return (
		...
		>
			...
			<dialog
				...
				data-pb-delay={
					props.autoOpen === "delay" ? String(props.delay) : undefined
				}
				...
				<>
					<Group title="Modal">
						<TextField
							path="name"
							label="Nome"
							hint="Só para identificar no editor."
						/>
						<SwitchField
							path="previewInEditor"
							label="Mostrar conteúdo no editor"
						/>
						<p className="text-[11px] text-muted-foreground">
							Para abrir, use em qualquer botão, imagem ou container a ação
							“Abrir modal” e escolha este modal.
						</p>
					</Group>
					<Group title="Abrir automaticamente">
						...
						/>
						{autoOpen.value === "delay" ? (
							<NumberField path="delay" label="Segundos" max={120} />
						) : null}
						{autoOpen.value === "exit" ? (
							<p className="text-[11px] text-muted-foreground">
								Abre quando o mouse sai pelo topo da janela (só em
								computadores).
							</p>
						) : null}
						{autoOpen.value !== "none" ? (
					...
					<Group title="Botão próprio">
						<SwitchField
							path="showTrigger"
							label="Mostrar botão que abre o modal"
						/>
						{showTrigger.value ? (
							<>
...
export const Modal: ComponentDefinition<ModalProps> = {
	...
	rules: {
		canMoveIn: (incoming) =>
			incoming.every(
				(n) =>
					!TOP_LEVEL_TYPES.has(n.data.name) &&
					n.data.name !== "Page" &&
					n.data.name !== "Modal",
			),
	},
	...
	css: (id, p) => {
		...
		sheet
			...
			.set("align-items", p.triggerAlign);
		...
		sheet
			...
			.set(
				"backdrop-filter",
				p.backdropBlur > 0 ? `blur(${p.backdropBlur}px)` : undefined,
			);
		sheet
			.rule(" .pb-modal[open]")
			.set("animation", "pb-modal-in .22s ease-out");
		sheet
