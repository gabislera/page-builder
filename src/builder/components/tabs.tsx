...
import { BackgroundFields, BorderFields, BoxFields, SidesField, TypographyFields } from "../controls/groups.tsx";
...
import { corners, defaultBackground, defaultBorder, defaultBox, defaultTypography, sides } from "../core/defaults.ts";
...
import type { Background, Border, Box, Length, Sides, Typography } from "../core/style-types.ts";
...
const VARIANTS: Record<TabsVariant, Partial<TabsProps>> = {
  segmented: {
    ...
    tabPadding: responsive(sides("10px", "18px"), undefined, sides("8px", "14px")),
  },
  underline: {
    listBackground: "",
    activeColor: C.text,
    activeBackground: "",
    tabPadding: responsive(sides("14px", "4px"), undefined, sides("12px", "2px")),
  },
  pills: {
    listBackground: "",
    activeColor: "#ffffff",
    activeBackground: C.primary,
    tabPadding: responsive(sides("10px", "20px"), undefined, sides("8px", "16px")),
  },
};
...
function TabsStatic({ id, props, children, rootRef }: NodeViewProps<TabsProps>) {
  ...
  const tabs: TabInfo[] = items.map((child, i) => {
    const p = isValidElement<{ props?: { label?: string; icon?: string } }>(child) ? child.props.props : undefined;
    return {
      id: String(i),
...
function TabsMarkup({
  ...
  return (
    <div
      ...
      className={nodeClassName(id, `pb-tabs pb-tabs-${props.variant}`, props.box)}
      data-pb-node={id}
      data-pb-tabs=""
    >
      ...
      <div className="pb-tabs-panels">
        ...
        {onSelect && empty ? <div className="pb-placeholder">Adicione abas no painel</div> : null}
      </div>
    </div>
  );
}
...
function TabButton({
  ...
  const edit = useInlineEdit(tab.label, onRename && ((v) => onRename(index, v)));
  return (
    <button
      type="button"
      role="tab"
      ...
      content={
        <Group title="Abas">
          <ChildItemsField
            ...
            create={(i) => h("TabItem", { label: `Aba ${i + 1}` }, [h("Text", { html: "<p>Conteúdo da aba.</p>" })])}
            min={1}
          />
        ...
        <>
          ...
          <Group title="Barra de abas">
            ...
            {props.orientation === "vertical" ? (
              <NumberUnitField path="navWidth" label="Largura da coluna" units={["px", "%"]} max={480} />
            ) : null}
            <NumberUnitField path="navGap" label="Espaço até o conteúdo" units={["px", "rem"]} max={120} />
            <ColorField path="listBackground" label="Fundo da barra" allowEmpty />
          </Group>
          <Group title="Aba" defaultOpen={false}>
            <TypographyFields base="tabTypography" withAlign={false} />
            <SidesField path="tabPadding" label="Espaço interno" units={["px", "rem"]} />
            <ColorField path="activeColor" label="Texto da aba ativa" />
            <ColorField path="activeBackground" label="Fundo da aba ativa" allowEmpty />
            {props.variant === "underline" ? <ColorField path="indicatorColor" label="Cor do sublinhado" /> : null}
          </Group>
          <Group title="Conteúdo" defaultOpen={false}>
            <SidesField path="panelPadding" label="Espaço interno" units={["px", "rem"]} />
            <NumberUnitField path="panelGap" label="Espaço entre elementos" units={["px", "rem"]} max={80} />
...
export const Tabs: ComponentDefinition<TabsProps> = {
  ...
  css: (id, p) => {
    ...
    sheet.rule(" > .pb-tabs-nav > .pb-tabs-list::-webkit-scrollbar").set("display", "none");
    ...
    btn
      ...
      .set("transition", `color .2s ${ease}, background-color .2s ${ease}, box-shadow .2s ${ease}`);
    if (stretch) btn.set("flex", "1");
    applyTypography(btn, p.tabTypography);
    sheet.rule(" > .pb-tabs-nav > .pb-tabs-list > .pb-tab-btn:hover").set("color", p.activeColor);
    const active = sheet.rule(" > .pb-tabs-nav > .pb-tabs-list > .pb-tab-btn.pb-tab-active");
    active.set("color", p.activeColor).set("background-color", p.activeBackground);
    sheet
    ...
    if (p.variant === "segmented") {
      ...
      active.set("box-shadow", "0 1px 2px rgba(0,0,0,.06), 0 2px 8px -2px rgba(0,0,0,.12)");
    } else if (p.variant === "underline") {
      list
        .set("gap", vertical ? "2px" : "28px")
        .set(vertical ? "border-left" : "border-bottom", `1px solid ${C.border}`);
      btn.set("border-radius", "0");
      active.set("box-shadow", vertical ? `inset 2px 0 0 ${p.indicatorColor}` : `inset 0 -2px 0 ${p.indicatorColor}`);
...
function TabItemView({ id, props: _props, children, rootRef }: NodeViewProps<TabItemProps>) {
  ...
  return (
    <div
      ...
      className={cn(nodeClassName(id, "pb-tab-panel", _props.box), active && "pb-tab-active")}
      data-pb-node={id}
    >
      {children}
      {isEditor && empty ? <div className="pb-placeholder">Arraste elementos para esta aba</div> : null}
      {isEditor ? (
        <RevealOnSelect
          id={id}
          onReveal={(selected) => {
...
export const tabsSpec = (): NodeSpec =>
  h(
    ...
    ["Visão geral", "Detalhes", "Perguntas"].map((label) =>
      h("TabItem", { label }, [h("Text", { html: `<p>Conteúdo da aba ${label}.</p>` })]),
    ),
  );
