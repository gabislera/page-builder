...
import { BackgroundFields, BoxFields, ShadowFields, SidesField } from "../controls/groups.tsx";
...
import { defaultBackground, defaultBox, defaultShadow, sides } from "../core/defaults.ts";
...
import { applyBackground, applyBox, createSheet, shadowToCss, sidesToCss } from "../core/style-engine.ts";
import type { Background, Box, Length, Shadow, Sides } from "../core/style-types.ts";
...
function HeaderView({ id, props, children, rootRef }: NodeViewProps<HeaderProps>) {
  ...
  return (
    ...
    >
      ...
      <div className="pb-header-inner">
        {children}
        {isEditor && empty ? <div className="pb-placeholder">Arraste Logo, Menu e Botão para cá</div> : null}
      </div>
    </header>
  );
}
      ...
      content={
        <>
          <Group title="Layout">
            ...
            <NumberUnitField path="minHeight" label="Altura mínima" units={["px"]} max={240} />
          ...
          <Group title="Ao rolar a página">
            ...
            {sticky.value ? (
              <>
                {transparent.value ? (
                  <>
                    <ColorField path="scrolledBackground" label="Fundo depois de rolar" allowEmpty />
                    <ColorField path="scrolledTextColor" label="Texto depois de rolar" allowEmpty />
                  </>
                ) : null}
                <SwitchField path="shadowOnScroll" label="Sombra ao rolar" />
                <SwitchField path="hideOnScrollDown" label="Esconder ao rolar para baixo" />
                <SwitchField path="shrinkOnScroll" label="Diminuir ao rolar" />
                {shrink.value ? (
                  <NumberUnitField path="shrinkPadding" label="Espaço vertical ao rolar" units={["px"]} max={60} />
                ) : null}
              </>
            ) : null}
          </Group>
      ...
      style={
        <>
          ...
          <Group title="Borda e sombra" defaultOpen={false}>
            ...
            {border.value ? <ColorField path="borderColor" label="Cor da linha" /> : null}
...
function headerCss(id: string, p: HeaderProps): string {
  ...
  root
    ...
    .set("border-bottom", p.borderBottom ? `1px solid ${p.borderColor}` : undefined)
    .set("box-shadow", shadowToCss(p.shadow))
    .set("transition", "background-color .3s ease,box-shadow .3s ease,padding .3s ease,transform .3s ease");
  applyBackground(root, p.background);
  applyBox(sheet, { ...p.box, padding: undefined, width: undefined, maxWidth: undefined }, "block");
    ...
    if (p.sticky) {
      ...
      if (p.scrolledTextColor) {
        for (const sel of [
          ...
          sheet.rule(`[data-pb-live].pb-scrolled${sel}`).set("color", p.scrolledTextColor);
        }
      }
    }
  } else if (p.sticky) {
  ...
  if (p.sticky) {
    if (p.shadowOnScroll) scrolled.set("box-shadow", "0 8px 24px -12px rgba(0,0,0,.25)");
    if (p.shrinkOnScroll) scrolled.set("padding-top", p.shrinkPadding).set("padding-bottom", p.shrinkPadding);
    if (p.hideOnScrollDown) sheet.rule("[data-pb-live].pb-header-hidden").set("transform", "translateY(-100%)");
  }
