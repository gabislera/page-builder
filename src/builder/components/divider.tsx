...
import { NumberUnitField, SegmentedField, SelectField } from "../controls/inputs.tsx";
    ...
    <SettingsTabs
      style={
        <Group title="Linha">
          ...
          />
          <NumberUnitField path="thickness" label="Espessura" units={["px"]} max={20} />
          <NumberUnitField path="length" label="Comprimento" units={["%", "px"]} max={100} />
          <SegmentedField
            path="align"
            label="Alinhamento"
            options={[
...
export const Divider: ComponentDefinition<DividerProps> = {
  ...
  css: (id, p) => {
    ...
    sheet
      ...
      .set("border-top-width", p.thickness, (t) => (p.style === "double" ? `calc(${t} * 3)` : t))
