	...
	return (
		<div
			...
			className={nodeClassName(
				id,
				`pb-gallery pb-gallery-${props.layout}`,
				props.box,
			)}
			data-pb-node={id}
				...
				<>
					<Group title="Imagens">
						<ListField<GalleryItem>
							...
							itemLabel={(item, i) =>
								item.caption || item.alt || `Imagem ${i + 1}`
							}
							renderItem={(itemPath) => (
								<>
									<MediaPathField
			...
			style={
				<>
					...
					<Group title="Legendas">
						...
						/>
						{captions.value !== "none" ? (
							<TypographyFields base="captionTypography" />
						) : null}
					</Group>
				</>
			}
...
export const Gallery: ComponentDefinition<GalleryProps> = {
	...
	css: (id, p) => {
		...
		const overlayHover =
			p.hoverEffect === "overlay" || p.hoverEffect === "zoom-overlay";
		...
		if (grid) {
			root
				...
				.set(
					...
					p.columns,
					(n) => `repeat(${Math.max(1, n)}, minmax(0, 1fr))`,
				)
				.set("gap", p.gap);
		} else {
			root
		...
		media
			...
			.set(
				"aspect-ratio",
				grid && p.ratio !== "original" ? p.ratio : undefined,
			);
		sheet.rule(" a.pb-gallery-media").set("cursor", "zoom-in");
		sheet
			.rule(" a.pb-gallery-media:not([data-pb-lightbox])")
			.set("cursor", "pointer");
		sheet
		...
		if (zoom)
			sheet
				.rule(" .pb-gallery-media:hover img")
				.set("transform", "scale(1.06)");
		...
		overlay
			...
			.set(
				"background",
				`color-mix(in srgb, ${p.overlayColor} 40%, transparent)`,
			);
		if (overlayHover)
			sheet.rule(" .pb-gallery-media:hover::after").set("opacity", "1");
		...
		cap
			...
			.set(
				"background",
				`linear-gradient(to top, color-mix(in srgb, ${p.overlayColor} 70%, transparent), transparent)`,
			)
		...
		if (overlayHover) {
			...
			sheet
				.rule(" .pb-gallery-media:hover .pb-gallery-overlay-cap")
				.set("opacity", "1")
				.set("transform", "none");
		}
