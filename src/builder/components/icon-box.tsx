...
function IconBoxView({
	id,
	props,
	rootRef,
	onPropChange,
}: NodeViewProps<IconBoxProps>) {
	const ctx = useRender();
	...
	const description = useInlineEdit(
		props.description,
		onPropChange && ((v) => onPropChange("description", v)),
		{ multiline: true },
	);
	const more = useInlineEdit(
		props.linkText,
		onPropChange && ((v) => onPropChange("linkText", v)),
	);
	...
	const content = (
		<>
			...
			<div className="pb-iconbox-content">
				...
				{props.linkText ? (
					<span
						ref={more.ref as React.Ref<HTMLSpanElement>}
						className="pb-iconbox-more"
						{...more.attrs}
					>
			...
			style={
				<>
					<Group title="Layout">
						...
						<NumberUnitField
							path="iconSpacing"
							label="Espaço entre ícone e texto"
							units={["px", "em"]}
							max={80}
						/>
						<NumberUnitField
...
export const IconBox: ComponentDefinition<IconBoxProps> = {
	...
	css: (id, p) => {
		...
		if (p.hover.enabled) {
			...
			const transforms = [
				p.hoverLift && parseFloat(p.hoverLift) !== 0
		...
		applyIconStyle(
			sheet,
			" .pb-iconbox-icon",
			":hover .pb-iconbox-icon",
			p.icon,
		);

		sheet
			.rule(" .pb-iconbox-content")
			.set("min-width", "0")
			.set("max-width", "100%");
