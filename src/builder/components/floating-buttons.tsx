...
import { DEVICES, hasOwn, type Responsive, resolve, responsive } from "../core/responsive.ts";
...
function FloatingButtonsView({ id, props, rootRef }: NodeViewProps<FloatingButtonsProps>) {
  const isEditor = useIsEditor();
  const ctx = useRender();
  const { backToTop: top, contact } = props;
  const link =
    contact.mode === "whatsapp"
      ? actionLink({ type: "whatsapp", phone: contact.phone, message: contact.message }, ctx)
      : actionLink(contact.action, ctx);
  const contactClass = contact.pulse ? "pb-float-btn pb-float-contact pb-float-pulse" : "pb-float-btn pb-float-contact";
  const lock: CSSProperties | undefined = isEditor ? { pointerEvents: "none" } : undefined;
  ...
  return (
    ...
    >
      {isEditor ? <span style={EDITOR_LABEL}>Botões flutuantes (fixos na página publicada)</span> : null}
      {top.enabled ? (
        <button
          type="button"
          className={isEditor ? "pb-float-btn pb-float-top pb-visible" : "pb-float-btn pb-float-top"}
      ...
      content={
        <>
          <Group title="Botão de contato">
            ...
            {contactEnabled.value ? (
              <>
                ...
                {mode.value === "whatsapp" ? (
                  <>
                    <TextField path="contact.phone" label="Número com DDI e DDD" hint="Ex.: 5511999998888" />
                    <TextAreaField path="contact.message" label="Mensagem inicial" rows={2} />
                  </>
                ) : (
          ...
          <Group title="Voltar ao topo">
            ...
            {topEnabled.value ? (
              <NumberField path="backToTop.showAfter" label="Aparece após rolar (px)" max={2000} step={50} />
            ) : null}
          </Group>
        </>
      }
      style={
        <>
          <Group title="Posição">
            ...
            />
            <NumberUnitField path="bottom" label="Distância da base" units={["px"]} max={200} />
            <NumberUnitField path="offset" label="Distância da lateral" units={["px"]} max={200} />
            <NumberField path="zIndex" label="Camada (z-index)" max={9999} />
          </Group>
          <Group title="Tamanho">
            <NumberUnitField path="size" label="Tamanho do botão" units={["px"]} min={32} max={96} />
            <NumberUnitField path="iconSize" label="Tamanho do ícone" units={["px"]} min={12} max={64} />
...
export const FloatingButtons: ComponentDefinition<FloatingButtonsProps> = {
  ...
  css: (id, p) => {
    ...
    for (const device of DEVICES) {
      ...
      root.setOn(device, side, resolve(p.offset, device)).setOn(device, side === "right" ? "left" : "right", "auto");
    }
    ...
    btn
      ...
      .set("transition", "transform .2s ease, opacity .25s ease, visibility .25s ease");
    ...
    sheet.rule(" .pb-float-top.pb-visible").set("opacity", "1").set("visibility", "visible").set("transform", "none");
    sheet.rule(" .pb-float-contact").set("color", p.contact.iconColor).set("background-color", p.contact.background);
    sheet
      .rule(" .pb-float-pulse")
      .set("animation", "pb-float-ring 2s ease-out infinite")
      .set("--pb-ring", p.contact.background);
    applyBox(sheet, { ...p.box, width: undefined, maxWidth: undefined, alignSelf: undefined }, "flex");
    return (
      sheet.toString() +
