...
function IconListView({ id, props, rootRef, onPropChange }: NodeViewProps<IconListProps>) {
  const ctx = useRender();
  return (
    <ul
          ...
          <>
            {item.icon ? (
              <span className="pb-iconlist-icon">
                <IconView name={item.icon} strokeWidth={props.iconStrokeWidth} />
              </span>
            ) : null}
            <InlineText
              className="pb-iconlist-text"
              value={item.text}
              onCommit={onPropChange && ((v) => onPropChange(`items.${i}.text`, v))}
            />
          </>
        );
        return (
      ...
      content={
        <>
          ...
          <Group title="Layout">
            ...
            <NumberUnitField path="gap" label="Espaço entre itens" units={["px", "em"]} max={80} />
          </Group>
        </>
      }
      style={
        <>
          <Group title="Ícone">
            <NumberUnitField path="iconSize" label="Tamanho" units={["px", "em"]} max={80} />
            <ColorField path="iconColor" label="Cor" />
            <NumberField path="iconStrokeWidth" label="Espessura do traço" min={0.5} max={4} step={0.25} />
            <NumberUnitField path="iconGap" label="Espaço até o texto" units={["px", "em"]} max={48} />
            <SegmentedField
              path="iconAlign"
          ...
          <Group title="Texto">
            ...
            <ColorField path="hoverColor" label="Cor no hover (links)" allowEmpty />
          </Group>
...
export const IconList: ComponentDefinition<IconListProps> = {
  ...
  css: (id, p) => {
    ...
    sheet.rule(" .pb-iconlist-link:hover").set("color", p.hoverColor || undefined);
