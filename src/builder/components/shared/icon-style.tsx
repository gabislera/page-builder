...
import { DEVICES, type Device, type Responsive, responsive } from "../../core/responsive.ts";
...
export const defaultIconStyle = (overrides: Partial<IconStyle> = {}): IconStyle => ({
  name: "star",
...
export function applyIconStyle(sheet: StyleSheet, selector: string, hoverSelector: string, s: IconStyle) {
  ...
  sheet
    ...
    .set("line-height", "0")
    .set("color", s.color)
    .set("transition", "color .2s ease,background-color .2s ease,border-color .2s ease,transform .2s ease")
    ...
    .set("border", s.view === "framed" ? `${s.borderWidth} solid ${s.borderColor}` : undefined);
  sheet
    .rule(`${selector} svg`)
    .set("width", s.size)
    .set("height", s.size)
  ...
  sheet
    ...
    .set("background-color", s.view === "stacked" ? s.hoverBackground || undefined : undefined)
    .set("border-color", s.view === "framed" ? s.hoverBorderColor || undefined : undefined);
}
...
export function setPerDevice(rule: StyleRule, compute: (device: Device) => Record<string, string | undefined>) {
  ...
  for (const device of DEVICES) {
    ...
    for (const [prop, value] of Object.entries(current)) {
      if (device === "desktop" || value !== previous[prop]) rule.setOn(device, prop, value);
    }
    previous = current;
  }
}
...
export function IconStyleFields({ base }: { base: string }) {
  ...
  return (
    <>
      ...
      <NumberUnitField path={`${base}.size`} label="Tamanho" units={["px", "em", "rem"]} max={200} />
      {view.value !== "default" ? (
        <NumberUnitField path={`${base}.padding`} label="Espaço interno da forma" units={["px", "em"]} max={100} />
      ) : null}
      <ColorField path={`${base}.color`} label="Cor do ícone" />
      {view.value === "stacked" ? <ColorField path={`${base}.background`} label="Cor da forma" /> : null}
      {view.value === "framed" ? (
        <>
          <ColorField path={`${base}.borderColor`} label="Cor da borda" />
          <NumberUnitField path={`${base}.borderWidth`} label="Espessura da borda" units={["px"]} max={12} />
        </>
      ) : null}
      <NumberField path={`${base}.strokeWidth`} label="Espessura do traço" min={0.5} max={4} step={0.25} />
...
export function IconHoverFields({ base }: { base: string }) {
  ...
  return (
    <>
      <ColorField path={`${base}.hoverColor`} label="Cor do ícone no hover" allowEmpty />
      {view.value === "stacked" ? (
        <ColorField path={`${base}.hoverBackground`} label="Cor da forma no hover" allowEmpty />
      ) : null}
      {view.value === "framed" ? (
        <ColorField path={`${base}.hoverBorderColor`} label="Cor da borda no hover" allowEmpty />
      ) : null}
    </>
  );
}
