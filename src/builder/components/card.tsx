...
function positionOn(p: CardProps, d: Device): ImagePosition {
	...
	if (d === "mobile" && p.stackOnMobile && (pos === "left" || pos === "right"))
		return "top";
	return pos;
}
...
function CardView({
	...
	const badge =
		props.badge || badgeEdit.editing ? (
			<span
				ref={badgeEdit.ref as React.Ref<HTMLSpanElement>}
				className="pb-card-badge"
				{...badgeEdit.attrs}
			>
	...
	const media = props.showImage ? (
		<div className="pb-card-media">
			<picture>
				{props.image.mobile ? (
					<source
						media={DEVICE_MEDIA.mobile ?? undefined}
						srcSet={props.image.mobile}
					/>
				) : null}
				{props.image.tablet ? (
					<source
						media={DEVICE_MEDIA.tablet ?? undefined}
	...
	const ctaClass =
		props.ctaType === "button" ? "pb-btn pb-card-btn" : "pb-card-link";
		...
		<>
			...
			<div className="pb-card-body">
				...
				{props.title || title.editing ? (
					<TitleTag
						ref={title.ref as React.Ref<HTMLHeadingElement>}
				...
				{props.text || text.editing ? (
					<p
						ref={text.ref as React.Ref<HTMLParagraphElement>}
						className="pb-card-text"
						{...text.attrs}
					>
				...
				<>
					...
					<Group title="Texto">
						<TextField
							...
							label="Selo"
							placeholder="ex.: Novo"
							hint="Opcional. Aparece sobre a imagem."
						/>
						<TextAreaField
							path="title"
							label="Título"
			...
			style={
				<>
					<Group title="Layout">
						...
						<SidesField
							path="padding"
							label="Espaço interno do conteúdo"
							units={["px", "em", "%"]}
						/>
						<NumberUnitField
							path="gap"
							label="Espaço entre os elementos"
							units={["px", "em"]}
							max={60}
						/>
					</Group>
...
export const Card: ComponentDefinition<CardProps> = {
	...
	css: (id, p) => {
		...
		setPerDevice(root, (d) => {
			...
			return {
				"flex-direction":
					pos === "left" ? "row" : pos === "right" ? "row-reverse" : "column",
				"min-height":
					pos === "background" ? resolve(p.backgroundMinHeight, d) : "0px",
			};
		});
		if (p.cardAction.type !== "none") root.set("cursor", "pointer");

		const hoverTransform =
			p.hoverLift && Number.parseFloat(p.hoverLift) !== 0
				? `translateY(-${p.hoverLift})`
				: undefined;
		sheet
			.rule(":hover")
			.set("transform", hoverTransform)
		...
		if (p.showImage) {
			...
			if (p.imageZoom)
				sheet.rule(":hover .pb-card-media img").set("transform", "scale(1.05)");
		...
		setPerDevice(body, (d) => ({
			"justify-content":
				imageOn(d) === "background" ? "flex-end" : "flex-start",
		}));
		...
		return (
			sheet.toString() +
			(p.ctaType === "button"
				? buttonStyleCss(id, ".pb-card-btn", p.button)
				: "")
		);
	},
