...
import { BorderFields, BoxFields, ShadowFields, SidesField, TypographyFields } from "../controls/groups.tsx";
import {
  NumberUnitField,
  SegmentedField,
  SelectField,
...
function FaqIcon({ style }: { style: FaqProps["iconStyle"] }) {
  ...
  if (style === "plus") {
    return (
      <svg className="pb-faq-icon pb-faq-plus" viewBox="0 0 24 24" aria-hidden="true">
  ...
  return (
    <svg className="pb-faq-icon pb-faq-chevron" viewBox="0 0 24 24" aria-hidden="true">
      <path d="m6 9 6 6 6-6" />
    </svg>
  );
}
...
function FaqEntry({
  ...
  const question = useInlineEdit(item.question, onPropChange && ((v) => onPropChange(`items.${index}.question`, v)));
  const answer = useInlineEdit(item.answer, onPropChange && ((v) => onPropChange(`items.${index}.answer`, v)), {
    multiline: true,
  });
  ...
  return (
    <details className="pb-faq-item" name={name} open={open || undefined}>
      ...
      >
        ...
        <span className="pb-faq-q-text" ref={question.ref as React.Ref<HTMLSpanElement>} {...question.attrs}>
          {question.editing ? null : item.question}
        </span>
        {props.iconPosition === "right" ? icon : null}
      </summary>
      {item.answer || isEditor ? (
        <div className="pb-faq-a" ref={answer.ref as React.Ref<HTMLDivElement>} {...answer.attrs}>
...
function FaqView({ id, props, rootRef, onPropChange }: NodeViewProps<FaqProps>) {
  const isEditor = useIsEditor();
  ...
  return (
    ...
    >
      ...
      {isEditor && props.items.length === 0 ? <div className="pb-placeholder">Adicione perguntas no painel</div> : null}
    </div>
  );
}
            ...
            <ListField<FaqItem>
              ...
              renderItem={(itemPath) => (
                <>
                  ...
                  <TextAreaField path={`${itemPath}.answer`} label="Resposta" rows={4} />
                </>
              )}
            />
          </Group>
      ...
      style={
        <>
          <Group title="Itens">
            ...
            <ColorField path="openBackground" label="Fundo quando aberto" allowEmpty />
            <SidesField path="questionPadding" label="Espaço interno da pergunta" units={["px", "rem"]} />
            <SidesField path="answerPadding" label="Espaço interno da resposta" units={["px", "rem"]} />
          </Group>
          <Group title="Ícone">
            <SegmentedField
              path="iconStyle"
            ...
            {iconStyle !== "none" ? (
              <>
                ...
                />
                <NumberUnitField path="iconSize" label="Tamanho" units={["px", "em"]} max={64} />
...
export const Faq: ComponentDefinition<FaqProps> = {
  ...
  defaults: {
    items: [
      {
        ...
        answer: "O acesso é vitalício. Você pode assistir quando e quantas vezes quiser.",
      },
      {
        id: "faq2",
        question: "Tem garantia?",
        answer: "Sim! Você tem 7 dias de garantia incondicional. Se não gostar, devolvemos 100% do valor.",
      },
      {
        id: "faq3",
    ...
    gap: responsive("12px"),
    questionPadding: responsive(sides("18px", "20px"), undefined, sides("16px")),
    answerPadding: responsive(sides("0px", "20px", "18px"), undefined, sides("0px", "16px", "16px")),
    dividerStyle: "none",
    dividerColor: "#e4e4e7",
    dividerWidth: "1px",
  ...
  css: (id, p) => {
    ...
    if (p.dividerStyle !== "none") {
      sheet
        ...
        .set("border-bottom", `${p.dividerWidth} ${p.dividerStyle} ${p.dividerColor}`);
    }
    ...
    sheet.rule(" .pb-faq-item[open] .pb-faq-chevron").set("transform", "rotate(180deg)");
    sheet.rule(" .pb-faq-v").set("transform-origin", "center").set("transition", "transform .25s ease");
  ...
  fonts: (p) => [p.questionTypography.fontFamily, p.answerTypography.fontFamily],
};
