...
function LogoView({
	...
	rootRef,
	onPropChange,
}: NodeViewProps<LogoProps>) {
	const ctx = useRender();
	const site = ctx.site;
	const siteSrc =
		props.variant === "light"
			? site.logoLightUrl || site.logoUrl
			: site.logoUrl;
	...
	const content = src ? (
		...
		<InlineText
			...
			onCommit={onPropChange && ((v) => onPropChange("text", v))}
		/>
	);
						...
						{source.value === "custom" ? (
							<>
								<MediaPathField
									...
									label="Imagem"
									accept="image"
									hint="Sem imagem, o texto é exibido."
								/>
