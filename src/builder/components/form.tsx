...
import type { Action, Background, Border, Box, Length, Shadow, Typography } from "../core/style-types.ts";
...
const fieldName = (f: FormField, index: number) => slugify(f.name) || slugify(f.label) || `campo_${index + 1}`;
...
function FormView({ id, props, rootRef }: NodeViewProps<FormProps>) {
  ...
  const renderField = (f: FormField, index: number) => {
    ...
    if (f.type === "hidden") return <input key={f.id} type="hidden" name={name} value={f.value} />;
    ...
    if (f.type === "checkbox") {
      return (
        <label key={f.id} className="pb-form-check pb-form-full">
          <input type="checkbox" name={name} value="Sim" required={f.required} {...lock} />
          <span>
            {f.label}
            {required(f)}
          </span>
    ...
    if (f.type === "textarea") {
      ...
      control = (
        <select className="pb-form-input" name={name} required={f.required} aria-label={aria} defaultValue="" {...lock}>
      ...
      control = (
        <div className="pb-form-tel" data-pb-tel="" data-pb-ddi-default={country}>
          {f.showCountry ? (
            <select
              ...
              {...lock}
            >
              {COUNTRIES.some((c) => c.code === country) ? null : <option value={country}>+{country}</option>}
              {COUNTRIES.map((c) => (
    ...
    return (
      ...
      f.type === "tel" ? (
        ...
        <label key={f.id} className={f.type === "textarea" ? "pb-form-field pb-form-full" : "pb-form-field"}>
          {label}
          {control}
        </label>
      )
  ...
  return (
    ...
    >
      <input type="hidden" name="_pb_form" value={props.formName || "Formulário"} />
      <input type="text" name="_pb_hp" tabIndex={-1} autoComplete="off" aria-hidden="true" style={HONEYPOT_STYLE} />
      {props.tags.trim() ? <input type="hidden" name="tags" value={props.tags.trim()} /> : null}
      {multi ? (
        <>
          {props.fields.map((f, i) => (f.type === "hidden" ? renderField(f, i) : null))}
          ...
          <div className="pb-form-nav">
            <button
              ...
              data-pb-prev=""
              hidden={active === 0}
              onClick={isEditor ? () => setPreviewStep(id, active - 1) : undefined}
            >
              {props.prevText}
            </button>
            <button
      ...
      <div className="pb-form-msg pb-form-error" data-pb-error="" role="alert" hidden>
        {props.errorMessage}
      </div>
    </form>
  );
...
function SubmitButton({
  ...
  return (
    <button type="submit" className="pb-btn pb-form-submit" hidden={hidden} style={style} {...lock}>
...
function StepProgress({ props, steps, active }: { props: FormProps; steps: Step[]; active: number }) {
      ...
      <ol className="pb-form-progress pb-form-dots" aria-hidden="true">
        {steps.map((s, i) => (
          ...
          >
            ...
            {s.title ? <span className="pb-form-dot-label">{s.title}</span> : null}
          </li>
        ))}
      </ol>
    );
  ...
  return (
    <div
      ...
      style={{ "--pb-progress": `${((active + 1) / total) * 100}%` } as CSSProperties}
    >
...
function FieldItem({ itemPath, field }: { itemPath: string; field: FormField }) {
  return (
    <>
      ...
      />
      <TextField path={`${itemPath}.label`} label={field.type === "hidden" ? "Descrição" : "Rótulo"} />
      <TextField
        path={`${itemPath}.name`}
      ...
      {field.type === "hidden" ? (
        ...
        <>
          {field.type !== "checkbox" ? <TextField path={`${itemPath}.placeholder`} label="Texto de exemplo" /> : null}
          <SwitchField path={`${itemPath}.required`} label="Obrigatório" />
        </>
      )}
      {field.type === "select" ? (
        <TextAreaField path={`${itemPath}.options`} label="Opções (uma por linha)" rows={4} />
      ) : null}
      {field.type === "tel" ? (
        <>
          <SwitchField path={`${itemPath}.showCountry`} label="Mostrar seletor de DDI" />
          <SelectField
...
function RedirectField() {
  ...
  const action: Action =
    f.value?.type === "url" || f.value?.type === "page" ? f.value : { type: "url", url: "", newTab: false };
  return (
    <>
      <Field label="Destino">
        <SelectInput
          ...
          onChange={(t) =>
            f.set(t === "page" ? { type: "page", pageId: "", newTab: false } : { type: "url", url: "", newTab: false })
          }
        />
      </Field>
    ...
    return (
      <>
        <ListField<FormField>
          ...
          renderItem={(itemPath, item) => <FieldItem itemPath={itemPath} field={item} />}
        />
        <Button
          type="button"
          size="sm"
          ...
          onClick={() => setFields([stepMarker("Etapa 1"), ...fields, stepMarker("Etapa 2")])}
        >
    ...
    <Field label="Etapas">
      <ListEditor<StepGroup>
        ...
        renderItem={(g, i) => (
          <>
            {g.marker ? (
              <TextField path={`fields.${g.markerIndex}.label`} label="Título da etapa" />
            ) : (
              <Field label="Título da etapa">
                <Input
                  ...
                  onChange={(e) =>
                    setFields(flatten([{ ...g, marker: stepMarker(e.target.value) }, ...groups.slice(1)]))
                  }
                />
              </Field>
            )}
            <Field label="Campos da etapa">
              <ListEditor<FormField>
                ...
                onChange={(fn) => {
                  ...
                  setFields(
                    flatten(
                      groups.map((x) => (x.id === g.id ? { ...x, items: next.map((f) => ({ f, index: -1 })) } : x)),
                    ),
                  );
                }}
                itemLabel={fieldLabel}
                renderItem={(f, k) => (
                  <FieldItem itemPath={`fields.${g.items[k]?.index ?? fields.indexOf(f)}`} field={f} />
                )}
              />
              {g.items.length === 0 ? (
                <p className="text-[11px] text-muted-foreground">Etapa sem campos não aparece no formulário.</p>
              ) : null}
            </Field>
          </>
        )}
      ...
      content={
        <>
          <Group title="Formulário">
            <TextField path="formName" label="Nome do formulário" hint="Aparece nos leads recebidos." />
            <FieldsEditor />
          </Group>
          <StepsGroup />
          ...
          <Group title="Após o envio">
            ...
            {after.value === "redirect" ? (
              ...
              <>
                <TextAreaField path="successMessage" label="Mensagem de sucesso" />
                <ColorField path="successColor" label="Cor da mensagem" />
                <SwitchField path="hideOnSuccess" label="Esconder campos após enviar" />
              </>
            )}
            <TextField path="errorMessage" label="Mensagem de erro" />
          </Group>
          <Group title="Rastreamento" defaultOpen={false}>
            <TextField path="tags" label="Tags" placeholder="ex.: lead, webinar" hint="Separadas por vírgula." />
      ...
      style={
        <>
          <Group title="Layout">
            ...
            <NumberUnitField path="gap" label="Espaço entre campos" units={["px", "rem"]} max={60} />
            <SegmentedField
              path="submitAlign"
          ...
          <Group title="Rótulos">
            ...
            {showLabels.value ? <TypographyFields base="labelTypography" withAlign={false} /> : null}
          </Group>
          <Group title="Campos">
            <NumberUnitField path="inputHeight" label="Altura" units={["px"]} min={28} max={80} />
            <NumberUnitField path="inputPaddingX" label="Espaço interno lateral" units={["px"]} max={40} />
...
function stepsCss(sheet: ReturnType<typeof createSheet>, id: string, p: FormProps) {
  ...
  sheet.rule(" .pb-form-nav").set("display", "flex").set("align-items", "center").set("gap", "12px");
  for (const btn of [" .pb-form-nav > .pb-form-next", " .pb-form-nav > .pb-form-submit"])
    sheet
      .rule(btn)
      .set("width", "auto")
      .set("margin-left", "auto")
      .set("flex", p.submitAlign, (a) => (a === "stretch" ? "1 1 auto" : "0 1 auto"));
  sheet
    .rule(" .pb-form-prev")
  ...
  sheet.rule(" .pb-form-dots > li.pb-done + li::before").set("background", accent);
  sheet
    .rule(" .pb-form-dot")
    .set("display", "grid")
  ...
  sheet
    ...
    .set("color", accent)
    .set("box-shadow", `0 0 0 4px color-mix(in srgb, ${accent} 15%, transparent)`);
  sheet
...
export const Form: ComponentDefinition<FormProps> = {
  ...
  css: (id, p) => {
    ...
    sheet
      .rule(" .pb-form-fields")
      .set("display", "grid")
      .set("grid-template-columns", p.columns, (n) => `repeat(${n}, minmax(0, 1fr))`)
      .set("gap", p.gap);
    ...
    sheet.rule(" .pb-form-req").set("color", p.requiredColor).set("margin-left", "2px");
    ...
    sheet
      ...
      .set("resize", "vertical");
    sheet.rule(" .pb-form-input::placeholder").set("color", p.placeholderColor).set("opacity", "1");
    sheet
      .rule(" .pb-form-input:focus")
      .set("border-color", p.focusColor)
      .set("box-shadow", p.focusColor ? `0 0 0 3px color-mix(in srgb, ${p.focusColor} 22%, transparent)` : undefined);
    sheet.rule(" .pb-form-tel").set("display", "flex").set("gap", "8px");
    sheet.rule(" .pb-form-tel .pb-form-ddi").set("width", "auto").set("flex", "0 0 auto").set("padding", "0 6px");
    ...
    check.set("display", "flex").set("align-items", "flex-start").set("gap", "8px").set("cursor", "pointer");
    sheet
    ...
    sheet.rule(" .pb-form-submit").set("align-self", p.submitAlign).set("max-width", "100%");
    sheet.rule(" .pb-form-submit[disabled]").set("opacity", ".7").set("cursor", "progress");
    sheet
      .rule(" .pb-form-msg")
      .set("display", "block")
      .set("font-size", p.labelTypography.fontSize)
    ...
    return (
      sheet.toString() + buttonStyleCss(id, ".pb-form-submit", p.submit) + buttonStyleCss(id, ".pb-form-next", p.submit)
    );
  },
