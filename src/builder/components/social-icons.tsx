...
export function socialHref(network: string, url: string): string {
	...
	if (network === "phone" && !/^tel:/i.test(v))
		return `tel:${v.replace(/[^\d+]/g, "")}`;
	if (network === "whatsapp" && /^[\d\s()+-]+$/.test(v))
...
function SocialIconsView({
	...
	props,
	rootRef,
}: NodeViewProps<SocialIconsProps>) {
	return (
		<div
		...
		>
			{props.items.map((item) => {
				...
				const external =
					props.newTab && /^https?:/i.test(href) ? true : undefined;
				return (
					<a
						key={item.id}
...
export const SocialIcons: ComponentDefinition<SocialIconsProps> = {
	...
	css: (id, p) => {
		...
		link
			...
			.set(
				"transition",
				"color .2s ease,background-color .2s ease,transform .2s ease,opacity .2s ease",
			);
		sheet
			.rule(" .pb-social-link svg")
		...
		if (p.colorMode === "custom") {
			link
				...
				.set("background-color", filled ? p.background : undefined);
			hover
				.set("color", p.hoverIconColor || undefined)
				.set(
					"background-color",
					filled ? p.hoverBackground || undefined : undefined,
				);
		} else {
