...
function searchIcons(query: string): { names: string[]; total: number } {
	...
	for (const w of words) {
		for (const [pt, terms] of Object.entries(PT_TERMS)) {
			if (w.length >= 3 ? pt.startsWith(w) : pt === w)
				for (const t of terms) translated.add(t);
		}
	}
	...
	const consider = (name: string, label: string) => {
		...
		else if (words.every((w) => name.includes(w) || label.includes(w)))
			score = 2;
...
function IconPicker({
	...
	const options = useMemo(
		() => (allowNone && !searching ? ["", ...names] : names),
		[allowNone, searching, names],
	);
