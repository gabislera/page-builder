...
export function Field({ label, children, responsive, overridden, onReset, inline, hint, className }: FieldProps) {
  return (
    <div className={cn(inline ? "flex items-center justify-between gap-3" : "flex flex-col gap-1.5", className)}>
      <div className="flex min-h-5 items-center gap-1.5">
        <span className="text-[11px] font-medium text-muted-foreground">{label}</span>
        {responsive ? <DeviceBadge overridden={overridden} onReset={onReset} /> : null}
      </div>
...
function DeviceBadge({ overridden, onReset }: { overridden?: boolean; onReset?: () => void }) {
    ...
    <span className="flex items-center gap-0.5">
      ...
      {onReset ? (
        <Tooltip>
          ...
          <TooltipContent side="top">Voltar a herdar do dispositivo maior</TooltipContent>
        </Tooltip>
      ) : null}
    </span>
  );
