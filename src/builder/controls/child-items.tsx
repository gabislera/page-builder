		...
		<Field label={label}>
			<div className="flex flex-col gap-1.5">
				{items.map((item, index) => (
					<div
						key={item.id}
						className="flex h-8 items-center gap-1 rounded-md border border-border bg-card px-2"
					>
						<button
							type="button"
						...
						>
							...
							<span className="truncate">
								{itemLabel(item.props, index) || `Item ${index + 1}`}
							</span>
						</button>
						<IconAction
							title="Subir"
						...
						<IconAction
							title="Duplicar"
							onClick={() => duplicateNode(editor, item.id)}
						>
