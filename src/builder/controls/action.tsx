			...
			return (
				<>
					...
					<NewTab
						...
						onChange={(newTab) => onChange({ ...action, newTab })}
					/>
				</>
			);
		case "section":
			return (
				<SectionPicker
					...
					onChange={(sectionId) => onChange({ ...action, sectionId })}
				/>
			);
		case "page":
			return (
		...
		case "modal":
			return (
				<ModalPicker
					...
					onChange={(modalId) => onChange({ ...action, modalId })}
				/>
			);
		case "whatsapp":
			return (
				<>
					<Field label="Número com DDI e DDD" hint="Ex.: 5511999998888">
						<DebouncedInput
							value={action.phone}
							onChange={(phone) => onChange({ ...action, phone })}
						/>
					</Field>
	...
	const { sections } = useEditor((state) => ({
		sections: (state.nodes[ROOT_ID]?.data.nodes ?? []).map((id) => {
			...
			return {
				value: id,
				label:
					(node?.data.custom?.displayName as string) ||
	...
	const { modals } = useEditor((state) => ({
		modals: Object.values(state.nodes)
			...
			.map((n) => ({
				value: n.id,
				label:
					(n.data.custom?.displayName as string) || `Modal ${n.id.slice(0, 4)}`,
			})),
	}));
	if (modals.length === 0) {
		return (
			<p className="text-[11px] text-muted-foreground">
				Adicione um elemento Modal na página primeiro.
			</p>
		);
	}
	return (
