      ...
      <div className="flex items-center gap-2">
        <div className="flex-1">
          <NumberUnitInput
            ...
            units={units}
            keywords={keywords}
            onChange={(v, t) => onChange(Object.fromEntries(keys.map((k) => [k, v])) as Record<K, string>, t)}
          />
        </div>
      ...
      <div className="grid flex-1 grid-cols-2 gap-2">
        {keys.map((k) => (
          <div key={k} className="flex flex-col gap-1">
            <span className="text-[10px] text-muted-foreground">{labels[k]}</span>
            <NumberUnitInput
              slider={false}
              value={value[k]}
              units={units}
...
function LinkToggle({ linked, onClick }: { linked: boolean; onClick: () => void }) {
...
export function CornersField({ path, label }: { path: string; label: string }) {
  ...
  return (
    <Field label={label} responsive={f.responsive} overridden={f.overridden} onReset={f.reset}>
      <FourValues
...
export function TypographyFields({
  ...
  return (
    <>
      <SelectField path={join(base, "fontFamily")} label="Fonte" options={fontChoices({ inherit: true })} />
      <div className="grid grid-cols-1 gap-3">
        <NumberUnitField path={join(base, "fontSize")} label="Tamanho" units={["px", "rem", "em", "vw"]} max={120} />
        <SelectField path={join(base, "fontWeight")} label="Peso" options={WEIGHTS} />
      </div>
      {withColor ? <ColorField path={join(base, "color")} label="Cor" /> : null}
      {withAlign ? <SegmentedField path={join(base, "textAlign")} label="Alinhamento" options={TEXT_ALIGN} /> : null}
      <NumberUnitField path={join(base, "lineHeight")} label="Altura da linha" unitless max={3} step={0.05} />
      <NumberUnitField
...
export function BackgroundFields({ base }: { base: string }) {
  ...
  return (
    <>
      ...
      />
      {type.value === "color" ? <ColorField path={join(base, "color")} label="Cor" /> : null}
      {type.value === "gradient" ? (
        <>
          <SegmentedField
          ...
          <NumberField path={join(base, "gradient.fromPosition")} label="Posição inicial (%)" max={100} />
          <ColorField path={join(base, "gradient.to")} label="Cor final" />
          <NumberField path={join(base, "gradient.toPosition")} label="Posição final (%)" max={100} />
          <NumberField path={join(base, "gradient.angle")} label="Ângulo (°)" max={360} />
        </>
      ) : null}
      {type.value === "image" ? (
        <>
          <MediaPathField path={join(base, "image.url")} label="Imagem" accept="image" />
          <SelectField
          ...
          <SelectField
            ...
            label="Posição"
            options={["center center", "top center", "bottom center", "center left", "center right"].map((v) => ({
              value: v,
              label: POSITION_LABEL[v],
            }))}
          />
          <SwitchField path={join(base, "image.repeat")} label="Repetir" />
          <SwitchField path={join(base, "image.fixed")} label="Fixo ao rolar (parallax)" />
          <ColorField path={join(base, "overlay")} label="Sobreposição" allowEmpty />
        </>
      ) : null}
    </>
  );
...
export function BorderFields({ base }: { base: string }) {
  ...
  return (
    <>
      ...
      {style.value !== "none" ? (
        <>
          <SidesField path={join(base, "width")} label="Espessura" units={["px"]} />
...
export function ShadowFields({ base, text }: { base: string; text?: boolean }) {
  ...
  return (
    <>
      <SwitchField path={join(base, "enabled")} label={text ? "Sombra no texto" : "Sombra"} />
      {enabled.value ? (
        <>
          <ColorField path={join(base, "color")} label="Cor" />
          <NumberField path={join(base, "x")} label="Horizontal" min={-50} max={50} />
          <NumberField path={join(base, "y")} label="Vertical" min={-50} max={50} />
          <NumberField path={join(base, "blur")} label="Desfoque" max={100} />
          {text ? null : (
            <>
              <NumberField path={join(base, "spread")} label="Expansão" min={-50} max={50} />
...
export function HoverFields({
  ...
  return (
    <>
      <SwitchField path={join(base, "enabled")} label="Efeito ao passar o mouse" />
      {enabled.value ? (
        <>
          {withBackground ? <ColorField path={join(base, "background")} label="Fundo" allowEmpty /> : null}
          {withColor ? <ColorField path={join(base, "color")} label="Texto" allowEmpty /> : null}
          <ColorField path={join(base, "borderColor")} label="Borda" allowEmpty />
          <NumberField path={join(base, "opacity")} label="Opacidade" max={1} step={0.05} />
          <NumberField path={join(base, "scale")} label="Zoom" min={0.8} max={1.3} step={0.01} />
          <NumberField path={join(base, "durationMs")} label="Duração (ms)" max={1000} step={50} />
        </>
      ) : null}
    </>
  );
...
export function BoxFields({ base = "box", withSize = true }: { base?: string; withSize?: boolean }) {
  return (
    <>
      <Group title="Espaçamento">
        <SidesField path={join(base, "margin")} label="Margem externa" units={["px", "%", "rem"]} />
        <SidesField path={join(base, "padding")} label="Espaço interno" units={["px", "%", "rem"]} />
      </Group>
      {withSize ? (
        <Group title="Tamanho">
          <NumberUnitField
          ...
          <NumberUnitField
            ...
            max={1600}
          />
          <NumberUnitField path={join(base, "minHeight")} label="Altura mínima" units={["px", "vh"]} max={1000} />
          <SegmentedField
      ...
      <Group title="Transformar" defaultOpen={false}>
        <NumberField path={join(base, "transform.rotate")} label="Girar (°)" min={-180} max={180} />
        <NumberField path={join(base, "transform.scale")} label="Escala" min={0.1} max={3} step={0.05} />
        <NumberUnitField
      ...
      <Group title="Identificação" defaultOpen={false}>
        <TextField path={join(base, "anchorId")} label="ID âncora" placeholder="ex.: oferta" />
...
function PositionGroup({ base }: { base: string }) {
  ...
  return (
    <Group title="Posição" defaultOpen={false}>
      <SelectField path={join(base, "position")} label="Posição" options={POSITIONS} />
      {active ? (
        <>
          <p className="text-[11px] text-muted-foreground">{POSITION_HINT[position.value]}</p>
          <SidesField
...
function ScrollEffectFields({ base }: { base: string }) {
  ...
  return (
    <>
      ...
      />
      {type.value === "parallax" ? (
        <NumberField path={join(base, "scrollEffect.speed")} label="Intensidade" min={-1} max={1} step={0.05} />
      ) : null}
    </>
  );
}
        ...
        {VISIBILITY_DEVICES.map(({ key, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              <Switch checked={effective} onCheckedChange={(v) => set({ ...value, [key]: v })} />
            </label>
          );
        })}
      </div>
      <p className="flex items-center gap-1 text-[10px] text-muted-foreground">
        <Eye className="size-3" /> Elementos ocultos continuam no editor em modo translúcido.
      </p>
    </Field>
  );
}
