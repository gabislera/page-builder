  ...
  return (
    <div className="flex flex-col gap-2">
          ...
          >
            {value ? (
              ...
              <span className="flex flex-col items-center gap-1 text-[11px] text-muted-foreground">
                {accept === "video" ? <Video className="size-5" /> : <ImageIcon className="size-5" />}
      ...
      <div className="flex items-center gap-1.5">
        <DebouncedInput value={value} onChange={onChange} placeholder="ou cole uma URL" />
        {value ? (
          <button
            type="button"
            title="Remover"
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        <span>{upload.isPending ? "Enviando..." : "Arraste um arquivo ou clique para enviar"}</span>
        <input
          ref={inputRef}
          type="file"
      ...
      {accept === "image" ? (
        <label className="-mt-2 flex items-center gap-2 text-xs text-muted-foreground">
          ...
          Otimizar imagens (WebP, até 2400 px). Desligue para enviar o arquivo original.
        </label>
      ) : null}
      <div className="grid max-h-[50vh] grid-cols-4 gap-3 overflow-y-auto">
        {assets.isLoading ? <p className="col-span-4 text-sm text-muted-foreground">Carregando...</p> : null}
        {!assets.isLoading && items.length === 0 ? (
          <p className="col-span-4 text-sm text-muted-foreground">Nenhum arquivo ainda.</p>
        ) : null}
        {items.map((a) => (
          <div
            key={a.id}
          ...
          >
            <button type="button" className="size-full" onClick={() => onSelect(a.url)} title={a.name}>
              {a.mimeType.startsWith("video/") ? (
                <video src={a.url} className="size-full object-cover" muted />
              ) : (
                <img src={a.url} alt={a.name} className="size-full object-contain" loading="lazy" />
              )}
            </button>
            <Button
              size="icon"
