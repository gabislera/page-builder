...
export function RichTextField({ path, label }: { path: string; label: string }) {
  ...
  if (inline.nodeId === id && inline.editor) {
    return (
      <Field label={label} hint="Editando direto no canvas. Clique fora do texto para concluir.">
        <RichToolbar editor={inline.editor} />
      </Field>
    );
  }
  return (
    <Field label={label} hint="Dica: dê dois cliques no texto para editar direto no canvas.">
      <PanelEditor key={id} value={value} onChange={(html) => set(path, html, 600)} />
    </Field>
  );
}

function PanelEditor({ value, onChange }: { value: string; onChange: (html: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-1 border-b border-input bg-muted/40 p-1">
      <div className="flex flex-wrap items-center gap-0.5">
        ...
        {btn(state.italic, () => chain().toggleItalic().run(), Italic, "Itálico")}
        {btn(state.underline, () => chain().toggleUnderline().run(), Underline, "Sublinhado")}
        {btn(state.strike, () => chain().toggleStrike().run(), Strikethrough, "Riscado")}
        {btn(state.bullet, () => chain().toggleBulletList().run(), List, "Lista")}
        {btn(state.ordered, () => chain().toggleOrderedList().run(), ListOrdered, "Lista numerada")}
        {btn(state.align === "left", () => chain().setTextAlign("left").run(), AlignLeft, "Esquerda")}
        {btn(state.align === "center", () => chain().setTextAlign("center").run(), AlignCenter, "Centro")}
        {btn(state.align === "right", () => chain().setTextAlign("right").run(), AlignRight, "Direita")}
        {state.link ? (
          btn(true, () => chain().unsetLink().run(), LinkIcon, "Remover link")
        ) : (
          <LinkButton onApply={(href) => chain().setLink({ href }).run()} />
        )}
        {btn(false, () => chain().toggleHighlight({ color: "#fde68a" }).run(), Highlighter, "Marca-texto")}
        {btn(false, () => chain().unsetAllMarks().run(), RemoveFormatting, "Limpar formatação")}
      </div>
      <div className="flex items-center gap-1">
        ...
        <select
          className="h-7 flex-1 rounded border border-input bg-transparent px-1 text-[11px] outline-none"
          value={fontChoices().find((f) => fontStack(f.value) === state.fontFamily)?.value ?? ""}
          onChange={(e) =>
            e.target.value ? chain().setFontFamily(fontStack(e.target.value)).run() : chain().unsetFontFamily().run()
          }
        >
        ...
        <ColorInput
          value={state.color}
          allowEmpty
          onChange={(c) => (c ? chain().setColor(c).run() : chain().unsetColor().run())}
        />
      </div>
    </div>
  );
