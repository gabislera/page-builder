...
import { ROOT_ID, type SectionTree, type SitePart, subtree } from "../core/tree.ts";
...
export function useSitePart(part: SitePart) {
  ...
  const { current } = useEditor((state) => {
    const id = (state.nodes[ROOT_ID]?.data.nodes ?? []).find((n) => state.nodes[n]?.data.name === PART_TYPE[part]);
    const node = id ? state.nodes[id] : null;
    return {
      current: node ? { id: node.id, isSite: Boolean(node.data.custom?.sitePart) } : null,
    };
  });
  ...
  const setScope = (scope: PartScope) => {
    ...
    if (scope === "site" && !current.isSite) {
      editor.actions.setCustom(current.id, (custom: Record<string, unknown>) => {
...
export function SitePartCard({ part }: { part: SitePart }) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-border bg-card/50 p-3">
      <span className="text-[11px] font-medium text-muted-foreground">Exibir em</span>
