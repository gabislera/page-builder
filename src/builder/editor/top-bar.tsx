...
export function TopBar({
	...
	const publish = async () => {
		...
		try {
			...
			await waitForSaved();
			const res = await services.publishPage(
				useSaveState.getState().affectedPageIds,
			);
			useSaveState
				.getState()
				.set({ publishedAt: res.publishedAt, affectedPageIds: [] });
			...
			<DropdownMenu>
				...
				<DropdownMenuContent className="max-w-72">
					<p className="p-2 text-xs text-muted-foreground">
						Esta página foi salva em outra aba ou por outra pessoa depois que
						você abriu.
					</p>
					<DropdownMenuItem onClick={() => window.location.reload()}>
						Recarregar a versão mais nova
					</DropdownMenuItem>
					<DropdownMenuItem onClick={requestFlush}>
						Sobrescrever com a minha versão
					</DropdownMenuItem>
				</DropdownMenuContent>
			</DropdownMenu>
		);
	...
	return new Promise<void>((resolve, reject) => {
		...
		const check = () => {
			...
			if (status === "error" || status === "conflict")
				return reject(new Error(error ?? "Salve antes de publicar"));
			if (Date.now() - started > timeoutMs)
				return reject(new Error("Tempo esgotado ao salvar"));
