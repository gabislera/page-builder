  ...
  return (
    <div className="flex flex-col py-2">
      {rootChildren.length === 0 ? (
        <p className="p-4 text-xs text-muted-foreground">A página ainda não tem seções.</p>
      ) : null}
      {rootChildren.map((id) => (
...
function LayerItem({ id, depth }: { id: string; depth: number }) {
  ...
  const { node, actions, connectors, isSelected, isHovered } = useEditor((state) => {
    ...
    return {
      node: n
        ? {
            ...
            isGlobal: Boolean((FEATURES.globalSections && n.data.custom?.isGlobal) || n.data.custom?.sitePart),
          }
        : null,
  ...
  return (
    <div>
      ...
      >
        ...
        <span className={cn("flex-1 truncate", node.hidden && "text-muted-foreground line-through")}>{node.name}</span>
        <button
          type="button"
      ...
      {open && hasChildren
        ? node.children.map((child) => <LayerItem key={child} id={child} depth={depth + 1} />)
        : null}
    </div>
  );
}
