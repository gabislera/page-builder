...
export function PageMetaPanel({
	...
	const seo = (patch: Partial<PageSeo>) =>
		setMeta((m) => ({ ...m, seo: { ...m.seo, ...patch } }));
	const tracking = (patch: Partial<PageTracking>) =>
		setMeta((m) => ({ ...m, tracking: { ...m.tracking, ...patch } }));
	...
	return (
		<div className="flex flex-col">
			...
			<Group title="SEO e compartilhamento">
				<Field
					label="Título da aba / Google"
					hint={`${(meta.seo.title ?? "").length}/60 caracteres recomendados`}
				>
					<Input
						className="h-8 text-xs"
				...
				<Field
					label="Descrição"
					hint={`${(meta.seo.description ?? "").length}/160 caracteres recomendados`}
				>
					<Textarea
						rows={3}
				...
				<Field label="Favicon">
					<MediaInput
						accept="image"
						value={meta.seo.faviconUrl ?? ""}
						onChange={(v) => seo({ faviconUrl: v })}
					/>
				</Field>
				<Field label="Imagem de compartilhamento (1200×630)">
					<MediaInput
						accept="image"
						value={meta.seo.ogImageUrl ?? ""}
						onChange={(v) => seo({ ogImageUrl: v })}
					/>
				</Field>
				<Field label="Esconder dos buscadores" inline>
					<Switch
						checked={Boolean(meta.seo.noIndex)}
						onCheckedChange={(v) => seo({ noIndex: v })}
					/>
				</Field>
			</Group>
			<Group title="Rastreamento" defaultOpen={false}>
				<Field label="Pixel do Facebook (ID)">
					<Input
						...
						onChange={(e) =>
							tracking({ facebookPixelId: e.target.value.trim() })
						}
					/>
				</Field>
