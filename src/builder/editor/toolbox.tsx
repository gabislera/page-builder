...
const C = resolver as Record<string, React.ComponentType<Record<string, unknown>>>;
...
const simple = (type: string): ToolboxItem => {
  ...
  return {
    ...
    icon: def.icon,
    create: () =>
      def.isCanvas ? <Element is={C[type]} canvas {...def.defaults} /> : <Element is={C[type]} {...def.defaults} />,
  };
};
...
const topBarItem = (): ToolboxItem => ({
  ...
  onClick: ({ actions, query }) => {
    ...
    const tree = query.parseReactElement(simple(TOP_BAR_TYPE).create()).toNodeTree();
    actions.addNodeTree(tree, ROOT_ID, 0);
    actions.selectNode(tree.rootNodeId);
  },
});

const columns = (n: number, label: string, icon: ToolboxIcon, template = ""): ToolboxItem => ({
  key: `columns-${n}-${template || "iguais"}`,
  label,
  icon,
  create: () => (
    <Element is={C.Container} canvas {...containerPresets.grid(n, template)} custom={{ displayName: "Colunas" }}>
      {Array.from({ length: n }, (_, i) => `col-${i}`).map((key) => (
        <Element key={key} is={C.Container} canvas {...containerPresets.stack} custom={{ displayName: "Coluna" }} />
      ))}
    </Element>
  ),
});
...
function modalItem(): ToolboxItem {
  ...
  const props = (type: string, patch: Record<string, unknown> = {}) =>
    deepMerge(structuredClone(COMPONENTS[type].defaults), patch) as Record<string, unknown>;
  ...
  return {
    ...
    icon: def.icon,
    create: () => (
      <Element is={C.Modal} canvas {...def.defaults} custom={{ displayName: "Pop-up" }}>
        <Element
          is={C.Heading}
          {...props("Heading", {
...
function specToElement(spec: NodeSpec, key?: string): ReactElement {
  ...
  const children = (spec.children ?? []).map((child, i) => specToElement(child, `c${i}`));
  return (
    <Element
      key={key}
      is={C[spec.type]}
...
export const TOOLBOX_GROUPS: { title: string; items: ToolboxItem[] }[] = [
  ...
  {
    title: "Mídia",
    items: [simple("Image"), simple("Video"), simple("Gallery"), simple("Map"), composite("Carousel", carouselSpec)],
  },
  {
    title: "Conversão",
    items: [
      simple("Form"),
      {
        ...
        icon: ListOrdered,
        create: () => specToElement({ type: "Form", props: multiStepFormProps() }),
      },
      {
        key: "faq",
      ...
      topBarItem(),
      ...["Countdown", "ProgressBar", "Testimonial", "PricingTable"].map(simple),
    ],
  },
  {
    title: "Avançado",
...
export function Toolbox() {
  ...
  return (
    <div className="flex flex-col gap-5 p-4">
      <p className="text-[11px] text-muted-foreground">Arraste um elemento para dentro de uma seção no canvas.</p>
      {TOOLBOX_GROUPS.map((group) => (
        <div key={group.title} className="flex flex-col gap-2">
          <h3 className="text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">{group.title}</h3>
            ...
            {group.items.map((item) => {
              ...
              return (
                <div
                  ...
                  title={item.hint}
                  onClick={item.onClick ? () => item.onClick?.(editor) : undefined}
                ...
                >
                  ...
                  <span className="px-1 text-[10px] leading-tight">{item.label}</span>
                </div>
              );
            })}
          </div>
