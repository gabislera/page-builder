...
import { ArrowDown, ArrowUp, ArrowUpToLine, Copy, Globe, GripVertical, Plus, Trash2 } from "lucide-react";
...
export function RenderNode({ render }: { render: ReactNode }) {
  ...
  const { id, dom, name, isHover, isSelected, parent, custom, isDeletable, isDraggable, connectors } = useNode(
    (node) => ({
      dom: node.dom,
      ...
      {showBar
        ? createPortal(
            <NodeBar
              ...
              isGlobal={Boolean((FEATURES.globalSections && custom?.isGlobal) || custom?.sitePart)}
...
function NodeBar({
  ...
  return (
    <>
      ...
      >
        {canDrag ? (
          <span ref={dragRef} title="Arrastar" style={{ ...iconBtn, cursor: "grab" }}>
            <GripVertical size={13} />
          </span>
        ) : null}
        <span style={{ padding: "0 4px", cursor: "pointer" }} onMouseDown={stop(() => editor.actions.selectNode(id))}>
          {isGlobal ? <Globe size={11} style={{ display: "inline", marginRight: 4, verticalAlign: -1 }} /> : null}
          {name}
        </span>
        {selected ? (
          <>
            ...
            {canDelete ? (
              <button type="button" title="Excluir" style={iconBtn} onMouseDown={stop(() => editor.actions.delete(id))}>
                <Trash2 size={12} />
              </button>
            ) : null}
          </>
