  ...
  toString(): string {
    ...
    for (const device of DEVICES) {
      ...
      for (const rule of this.rules) {
        ...
        block += `${rule.selector}{${[...decls].map(([p, v]) => `${p}:${v}`).join(";")}}`;
      }
      if (!block) continue;
...
export const cornersToCss = (c: Corners) => `${c.topLeft} ${c.topRight} ${c.bottomRight} ${c.bottomLeft}`;
...
export const shadowToCss = (s: Shadow) =>
  s.enabled ? `${s.inset ? "inset " : ""}${s.x}px ${s.y}px ${s.blur}px ${s.spread}px ${s.color}` : undefined;

export const textShadowToCss = (s: TextShadow) => (s.enabled ? `${s.x}px ${s.y}px ${s.blur}px ${s.color}` : undefined);
...
export function applyTypography(rule: StyleRule, t: Partial<Typography>) {
  rule
    ...
    .set("text-transform", t.textTransform, (v) => (v === "none" ? undefined : v))
    .set("font-style", t.fontStyle, (v) => (v === "normal" ? undefined : v))
    .set("text-decoration", t.textDecoration, (v) => (v === "none" ? undefined : v))
    .set("color", t.color);
}
...
export function applyBackground(rule: StyleRule, b: Background | undefined) {
  ...
  switch (b.type) {
    ...
    case "image": {
      ...
      if (b.overlay) layers.unshift(`linear-gradient(${b.overlay}, ${b.overlay})`);
      rule
...
export function applyBorder(rule: StyleRule, b: Border | undefined) {
  if (!b) return;
  if (b.style !== "none") {
    rule.set("border-style", b.style).set("border-width", b.width, sidesToCss).set("border-color", b.color);
  }
...
function setWithOverride<T>(
  ...
  for (const device of DEVICES) {
    ...
    if (own === undefined) continue;
    if (own !== inherited) {
      rule.setOn(device, property, own === fallback ? (cssForFallback ?? String(own)) : String(own));
    }
    inherited = own;
  }
}
...
function applyTransform(rule: StyleRule, box: Partial<Box>) {
  ...
  if (moved || parallax) {
    rule.set("translate", `${t?.translateX ?? "0px"} calc(${t?.translateY ?? "0px"} + var(--pb-py, 0px))`);
  }
  if (parallax) rule.set("--pb-parallax", String(box.scrollEffect?.speed ?? 0.3));
}
...
export function googleFontsHref(families: Iterable<string>): string | null {
  ...
  const params = list
    .map((f) => `family=${encodeURIComponent(f).replace(/%20/g, "+")}:wght@${GOOGLE_FONTS[f]}`)
    .join("&");
