...
import { corners, defaultBackground, defaultBorder, sides } from "../core/defaults.ts";
import { responsive } from "../core/responsive.ts";
import { C } from "../core/theme.ts";
import type { SectionTemplate } from "./sections.ts";

export const INTERACTIVE_CATEGORIES = ["Perguntas frequentes", "Abas", "Carrossel"] as const;
...
const FAQ = [
  ...
  ["Como recebo o acesso?", "Assim que o pagamento for aprovado, você recebe os dados de acesso no seu e-mail."],
];
...
const heroSlide = (title: string, text: string): NodeSpec =>
  h(
    "CarouselSlide",
    {
      ...
      padding: responsive(sides("64px", "80px"), undefined, sides("40px", "24px")),
      gap: responsive("20px"),
    },
    [
      h("Heading", {
...
export const INTERACTIVE_TEMPLATES: SectionTemplate[] = [
  {
    ...
    build: () =>
      h(
        ...
        [
          ...sectionTitle("Perguntas frequentes", "Tire suas dúvidas antes de começar."),
          h(
            "Accordion",
            { box: { maxWidth: responsive("800px") } },
            FAQ.map(([question, answer]) =>
              h("AccordionItem", { title: question }, [h("Text", { html: `<p>${answer}</p>` })]),
            ),
          ),
        ],
  ...
  },
  {
    ...
    build: () =>
      h(
        ...
        { alignItems: responsive("center"), gap: responsive("32px") },
        [
          ...sectionTitle("Tudo o que você precisa", "Organize os detalhes da sua oferta em abas."),
          h("Tabs", { justify: "center" }, [
            h("TabItem", { label: "Visão geral" }, [
              tabContent("Uma visão completa", "Apresente o principal benefício da sua oferta e para quem ela é."),
            ]),
            h("TabItem", { label: "Como funciona" }, [
              tabContent("Passo a passo simples", "Explique as etapas que o cliente percorre até chegar ao resultado."),
            ]),
            h("TabItem", { label: "Resultados" }, [
              tabContent("Resultados comprovados", "Mostre números, provas e depoimentos que reforçam a promessa."),
            ]),
          ]),
        ],
        "Abas",
        ...
        [
          h(
            ...
            [
              ...
              heroSlide("Uma segunda mensagem forte", "Use cada slide para um benefício, uma prova ou uma oferta."),
              heroSlide("Chamada final irresistível", "Reforce a oferta e a urgência antes do clique."),
            ],
          ),
        ],
        "Banner",
      ...
      h(
        ...
        [
          ...
          h(
            ...
            ["Destaque 1", "Destaque 2", "Destaque 3", "Destaque 4", "Destaque 5", "Destaque 6"].map(cardSlide),
          ),
        ],
        "Carrossel de cards",
      ),
