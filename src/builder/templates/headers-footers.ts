...
const divider = (color = "#ffffff1f") =>
	h("Divider", { color, box: { padding: responsive(sides("0px")) } });

const column = (children: NodeSpec[], gap = "16px") =>
	h(
		"Container",
		{ ...containerPresets.stack, gap: responsive(gap) },
		children,
		"Coluna",
	);

const linkColumn = (title: string, links: string[]) =>
	column([columnTitle(title), footerMenu(links)], "12px");

const brandColumn = () =>
	column([
		footerLogo(),
		footerText(
			"<p>Uma frase curta sobre a sua marca e o que ela entrega para os clientes.</p>",
		),
		footerSocials(),
	]);
	...
	},
	{
		...
		build: () =>
			h(
				...
				[
					logo({ maxHeight: responsive("48px", "36px", "32px") }),
					menu({ align: responsive("center") }),
				],
				"Cabeçalho",
			),
	},
						...
						[
							...
							column(
								[
									...
									footerText(
										"<p>Conteúdos e ofertas exclusivas direto no seu e-mail.</p>",
									),
									h("Form", {
										formName: "Newsletter",
										tags: "newsletter",
